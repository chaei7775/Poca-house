...
/* ===== POCA UI KILL SWITCH v9 ===== */
(function(){

  /* =========================
     1. HARD REMOVE SERVER BUTTON (BULLETPROOF)
  ==========================*/
  function killServerUI(){
    document.querySelectorAll('*').forEach(el=>{
      const t = (el.innerText || el.textContent || '').trim();

      // server save button removal
      ...
      // old yellow button fallback detection
      if(el.style && el.style.background && el.style.background.includes('yellow')){
        el.remove();
      }
    });
  }

  setInterval(killServerUI, 200);

  /* =========================
     2. BLOCK ALBA SHARE BUTTON COMPLETELY
  ==========================*/
  function blockAlbaShare(){
    const p = document.getElementById('result-popup');
    if(!p) return;

    p.querySelectorAll('button').forEach(b=>{
      const t = (b.innerText || '').trim();
      if(t.includes('자랑') || t.includes('share')){
        b.remove();
      }
    });
  }

  setInterval(blockAlbaShare, 200);

  /* =========================
     3. KILL ANY RECREATION (MUTATION DEFENSE)
  ==========================*/
  ...
  /* =========================
     4. SAFETY OVERRIDES (PREVENT RE-ATTACH)
  ==========================*/
  const killFn = (name)=>{
    if(window[name]){
  ...
  // block common dangerous re-render hooks
  killFn('addFirebaseTestPanel');
  killFn('showServerSyncToast'); // UI replaced
  killFn('initFirebaseUI');

  /* =========================
     5. OPTIONAL SAFE TIMELINE (DO NOT TOUCH GAME CORE)
  ==========================*/
  window.__poca_timeline = function(){
    try{
