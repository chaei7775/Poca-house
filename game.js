...
/* ===========================
   POCA ADMIN SAVE v1
   LEVEL 20 + FULL TEST SET
=========================== */
(function(){

window.applyAdminSave = function(){

  // 💰 재화
  localStorage.setItem('ph_coins', '2000000');
  window.coins = 2000000;

  // 🧍 레벨 (없을 수도 있어서 안전 fallback)
  window.playerLevel = 20;
  localStorage.setItem('ph_level', '20');

  // 🎴 UR 카드 전부 지급
  try{
    if (typeof CARDS !== 'undefined') {
      const urCards = CARDS.filter(c => c.grade === 'UR').map(c => c.id);
      localStorage.setItem('ph_owned', JSON.stringify(urCards));
      window.owned = urCards;
    }
  } catch(e){}
  ...
  localStorage.setItem('ph_bagItems', JSON.stringify(items));
  window.bagItems = items;

  // 🧠 UI 갱신 시도
  try{ if(typeof renderBag === 'function') renderBag(); }catch(e){}
  try{ if(typeof updateStaminaDisplay === 'function') updateStaminaDisplay(); }catch(e){}

  alert('🔥 ADMIN SAVE 적용 완료! (Lv20 / 200만코인 / UR / 재료세팅)');
};
