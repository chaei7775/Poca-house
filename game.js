...
(function(){
  ...
  function openShareUI(text, img, cb){
    const old = document.getElementById('share-popup');
    if(old) old.remove();

    const el = document.createElement('div');
    el.id = 'share-popup';
    el.style.cssText = 'position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.8);display:flex;align-items:center;justify-content:center;';
    el.innerHTML = `
      <div style="background:#1a1a2e;padding:20px;border-radius:16px;width:85%;max-width:320px;text-align:center;color:#fff;">
        ...
        <button id="doShare" style="width:100%;padding:10px;border:none;border-radius:10px;background:#FF6B9D;color:#fff;font-weight:900;">타임라인 올리기</button>
        <button onclick="document.getElementById('share-popup').remove()" style="margin-top:8px;width:100%;padding:8px;border:none;border-radius:10px;background:#333;color:#aaa;">닫기</button>
      </div>
    `;
    document.body.appendChild(el);
  ...
  // 🎴 가챠
  if(window.showGachaResult){
    const old = window.showGachaResult;
    window.showGachaResult = function(card){
      old.apply(this, arguments);
      setTimeout(()=>{
        const targetImg = card?.img || '';
        const popup = document.getElementById('gacha-single-fancy-overlay') || document.getElementById('gacha-overlay');
        if(popup){
          const btn = document.createElement('button');
          btn.textContent = '📢 자랑하기';
          btn.style.cssText = 'position:absolute;bottom:20px;left:50%;transform:translateX(-50%);padding:10px 14px;border:none;border-radius:12px;background:#FF6B9D;color:#fff;font-weight:900;';
          btn.onclick = ()=>openShareUI(`🎴 ${card.name} 획득!`, targetImg);
          popup.appendChild(btn);

          setTimeout(()=>btn.remove(), 1000); // 1초 후 사라짐 (UR용 느낌)
        }
      },50);
    }
  }
    ...
    window.showResult = function(){
      ...
      setTimeout(()=>{
        ...
        if(popup){
          const btn = document.createElement('button');
          btn.textContent = '📢 자랑하기';
          btn.style.cssText = 'margin-top:10px;width:100%;padding:10px;border:none;border-radius:12px;background:#C084FC;color:#fff;font-weight:900;';
          btn.onclick = ()=>openShareUI('🍔 알바 결과!', '');
          popup.appendChild(btn);
