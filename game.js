...
/* ===== POCA FIX v6 FINAL (TIMING CORRECTED) ===== */
(function(){

  /* 1) SERVER SAVE: 1s ONLY */
  if (window.showServerSyncToast) {
    window.showServerSyncToast = function(msg, ok){
      const old = document.getElementById('server-sync-toast');
      if (old) old.remove();

      const div = document.createElement('div');
      div.id = 'server-sync-toast';
      div.textContent = ok ? '✔ 저장' : '저장 실패';
      div.style.cssText =
        'position:fixed;top:6px;left:50%;transform:translateX(-50%);' +
        'z-index:99999;padding:3px 8px;font-size:10px;font-weight:900;' +
        'border-radius:999px;background:' + (ok ? '#22c55e' : '#ef4444') +
        ';color:#fff;pointer-events:none;box-shadow:0 2px 8px #0003;';
  ...
  function addTimeline(text, img){
    try{
      const list = JSON.parse(localStorage.getItem('ph_timeline') || '[]');
  ...
  function openShare(text, img){
    const old = document.getElementById('poca-share');
    if(old) old.remove();

    const el = document.createElement('div');
    el.id = 'poca-share';
    el.style.cssText = 'position:fixed;inset:0;z-index:99999;background:rgba(0,0,0,.85);display:flex;align-items:center;justify-content:center;';
    el.innerHTML = `
      <div style="background:#1a1a2e;padding:18px;border-radius:14px;width:85%;max-width:320px;color:#fff;text-align:center;">
        <div style="font-weight:900;margin-bottom:10px;">📢 자랑하기</div>
        ${img ? `<img src="${img}" style="width:100%;border-radius:10px;margin-bottom:10px;">` : ''}
        <div style="font-size:13px;margin-bottom:12px;">${text}</div>
        <button id="shareGo" style="width:100%;padding:10px;border:none;border-radius:10px;background:#FF6B9D;color:#fff;font-weight:900;">타임라인 올리기</button>
        <button onclick="document.getElementById('poca-share').remove()" style="margin-top:8px;width:100%;padding:8px;border:none;border-radius:10px;background:#333;color:#aaa;">닫기</button>
      </div>
    `;
    document.body.appendChild(el);

    document.getElementById('shareGo').onclick = function(){
      addTimeline(text, img);
      el.remove();
    };
  }

  /* 3) ATTACH HELPERS (5s popup + 1s button) */
  function attach(fnName, label, imgGetter){
    ...
    window[fnName] = function(){
      ...
      setTimeout(()=>{
        ...
        btn.style.cssText =
          ...
          'background:#C084FC;color:#fff;font-weight:900;z-index:99999;';
        ...
        setTimeout(()=>btn.remove(), 5000); // <<< FIXED 5s
  ...
  /* 4) GACHA / SCHOOL / CRAFT */
  attach('showGachaResult','🎴 가챠 결과', (a)=>a?.[0]?.img);
  attach('showResult','📚 성적 결과', ()=>'');

  /* craft variations */
  attach('showCraftResult','🧪 제작 결과', ()=>'');

  if(window.craftCloth){
    attach('craftCloth','🧪 제작 결과', ()=>'');

  }

  /* 5) WISH ITEMS */
  attach('showWishFragment','🧩 소원의 조각', ()=>'');

  attach('showWishCrystal','💎 소원의 결정', ()=>'');
