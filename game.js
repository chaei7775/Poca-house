...
function renderBag() {
  ...
  for (let i = 0; i < bagSlots; i++) {
    if (i < bagItems.length) {
      ...
      const itemVisual = item.img ? `<img src="${item.img}" style="width:34px;height:42px;object-fit:cover;border-radius:6px;" onerror="this.style.display='none'">` : `<div class="bag-item-emoji">${item.emoji}</div>`;
      html += `<div class="bag-slot has-item" onclick="showBagItemDetail(${i})">${itemVisual}<div class="bag-item-qty">x${item.qty}</div><div class="bag-item-name">${shortName}</div></div>`;
    } else {
...
function initBurgerAlba() {
  ...
  burgerStep = 0; burgerMenuIdx = -1; burgerStack = []; burgerGaugeLocked = false;
  renderBurgerStep();
}
function renderBurgerStep() {
  ...
  if (burgerStep >= menu.steps.length) {
    const earn = applyClothCoinBonus(CONFIG.alba.burger); coins += earn; albaDone++; saveAll();
    addPlayerExp(15);
    checkWishFragment();
...
function selectBurgerMenu(idx) { stopBurgerGauge(); burgerMenuIdx = idx; burgerStep = 0; burgerStack = []; burgerGaugeLocked = false; renderBurgerStep(); }
...
function tapBurgerGauge() {
  ...
  let earn;
  // v0.2 밸런스: 퍼펙트존 41~59 → 45.5~54.5로 절반 축소
  if (pos >= 45.5 && pos <= 54.5) { earn = 30; }
  else if (pos >= 35 && pos <= 65) { earn = 20; }
  else if (pos >= 25 && pos <= 75) { earn = 10; }
  else { earn = 0; }
  const finalEarn = applyClothCoinBonus(earn);
  if (pos >= 45.5 && pos <= 54.5) showResult('PERFECT', finalEarn);
  else if (pos >= 35 && pos <= 65) showResult('GREAT', finalEarn);
  else if (pos >= 25 && pos <= 75) showResult('COOL', finalEarn);
  else showResult('MISS', 0);
...
function initCafeAlba() {
  ...
  cafeStep = 0; cafeScores = []; cafeGaugeRunning = false; cafeGaugeLocked = false; renderCafeStep();
}
...
function renderCafeStep() {
  ...
  if (cafeStep === 0) {
    ...
    const bonus = applyClothCoinBonus(baseBonus);
...
const AFFECTION_THRESHOLDS = [
  ...
  { exp: 50,   stage: '친절', level: 5 }, // 신비의 섬 해금 기준
...
function openClothDetail(clothId, itemName) {
  ...
  overlay.innerHTML = `<div style="background:linear-gradient(135deg,#1a1a2e,#2d1b4e);border:2px solid ${item?.isGreat ? '#FFD700' : '#C084FC'};border-radius:20px;padding:20px;text-align:center;width:85%;max-width:300px;">
    <img src="${cloth.img}" style="width:100%;border-radius:12px;margin-bottom:12px;object-fit:cover;max-height:200px;" onerror="this.style.display='none'">
...
function openSewingWorkshop() {
  ...
  const listHtml = CLOTH_ITEMS.map(cloth => {
    ...
    return `<div onclick="openSewingRecipe('${cloth.id}')" style="display:flex;gap:10px;background:rgba(255,255,255,0.06);border:1.5px solid ${canCraft ? '#C084FC' : 'rgba(255,255,255,0.12)'};border-radius:14px;padding:10px;cursor:pointer;align-items:center;">
      <img src="${cloth.img}" onerror="this.style.display='none'" style="width:58px;height:74px;object-fit:cover;border-radius:10px;background:#2d1b4e;flex-shrink:0;">
...
function openSewingRecipe(clothId) {
  ...
  overlay.innerHTML = `<div style="background:linear-gradient(135deg,#1a1a2e,#2d1b4e);border:2px solid #C084FC;border-radius:22px;padding:18px;width:90%;max-width:340px;max-height:88vh;overflow-y:auto;text-align:center;">
    <img src="${cloth.img}" onerror="this.style.display='none'" style="width:100%;max-height:190px;object-fit:cover;border-radius:12px;margin-bottom:10px;background:#2d1b4e;">
...
function craftCloth(clothId) {
  ...
  if (result === 'fail') {
    showSewingResult('실패', '재료가 모두 소모됐어요 😭', '💥', '#FF6B9D');
  } else {
    ...
    showSewingResult(isGreat ? '대성공!' : '성공!', `${isGreat ? '✨ ' : ''}${cloth.name} 획득!<br>${statText}`, isGreat ? '✨👗✨' : '👗✨', isGreat ? '#FFD700' : '#C084FC');
  }
...
function showSewingResult(title, text, emoji, color) {
  ...
  overlay.innerHTML = `<div style="background:linear-gradient(135deg,#1a1a2e,#2d1b4e);border:2px solid ${color};border-radius:22px;padding:28px 22px;text-align:center;width:86%;max-width:300px;">
    <div style="font-size:46px;margin-bottom:8px;">${emoji}</div>
