...
function renderBag() {
  ...
  for (let i = 0; i < bagSlots; i++) {
    if (i < bagItems.length) {
      ...
      html += `<div class="bag-slot has-item" onclick="showBagItemDetail(${i})"><div class="bag-item-emoji">${item.emoji}</div><div class="bag-item-qty">x${item.qty}</div><div class="bag-item-name">${shortName}</div></div>`;
    } else {
...
function useDrinkFromBag(idx) {
  ...
  stamina = Math.min(100, stamina + up);
...
function drawOne() {
  ...
  // 카드 1장 획득 = 해당 포카 인연 경험치 +1 (중복 카드도 경험치로 인정)
  if (card.charId) addAffectionExp(card.charId, 1);
...
function confessChar(charId) {
  affectionExp[charId] = 4000;
  syncAffectionLegacy(charId);
  saveAll();
  const ch = CHARS[charId];
  showBagToast(`💝 ${ch.name} 인연이 MAX가 됐어요!`);
}
...
function initBurgerAlba() {
  ...
  burgerStep = 0; burgerMenuIdx = -1; burgerStack = []; burgerGaugeLocked = false; burgerCompleted = false; stopBurgerGauge();
  renderBurgerStep();
}
...
function selectBurgerMenu(idx) { stopBurgerGauge(); burgerMenuIdx = idx; burgerStep = 0; burgerStack = []; burgerGaugeLocked = false; burgerCompleted = false; renderBurgerStep(); }
...
function startBurgerGauge() {
  ...
  function tick() {
    ...
    if (!m || burgerGaugeLocked || burgerCompleted) { burgerGaugeAnimId = null; return; }
...
function tapBurgerGauge() {
  if (burgerGaugeLocked || burgerCompleted) return;
  burgerGaugeLocked = true;
  stopBurgerGauge();
  const pos = burgerGaugePos;
  let earn;
  if (pos >= 45.5 && pos <= 54.5) { earn = 30; showResult('PERFECT', 30); }
...
function initCafeAlba() {
  ...
  cafeStep = 0; cafeScores = []; cafeGaugeRunning = false; cafeGaugeLocked = false; cafeCompleted = false; stopCafeGaugeLoop(); renderCafeStep();
}
...
function renderCafeStep() {
  ...
  if (cafeStep === 0) {
    ...
    if (panel) panel.innerHTML = `<div style="text-align:center;color:#fff;"><div style="font-size:13px;color:#FFB3CC;margin-bottom:6px;">① 에스프레소 머신을 탭하세요!</div><div id="cafe-gauge-wrap" style="display:none;margin-top:8px;"><div style="font-size:12px;color:#aaa;margin-bottom:6px;">추출 타이밍!</div><div style="width:100%;height:28px;background:rgba(255,255,255,0.1);border-radius:14px;position:relative;overflow:hidden;border:1.5px solid rgba(255,255,255,0.2);cursor:pointer;" onclick="tapCafeGauge()"><div style="position:absolute;top:0;bottom:0;left:35%;width:30%;background:rgba(255,107,157,0.5);border-radius:4px;"></div><div style="position:absolute;top:0;bottom:0;left:45.5%;width:9%;background:rgba(255,215,0,0.7);border-radius:4px;"></div><div id="cafe-gauge-marker" style="position:absolute;top:3px;bottom:3px;width:5px;background:#fff;border-radius:3px;box-shadow:0 0 8px #fff;left:0%;"></div></div><button onclick="tapCafeGauge()" style="margin-top:8px;width:100%;padding:10px;background:linear-gradient(135deg,#FF6B9D,#C084FC);border:none;border-radius:10px;color:#fff;font-size:14px;font-weight:700;cursor:pointer;font-family:'Noto Sans KR',sans-serif;">TAP! ☕</button></div></div>`;
  } else if (cafeStep === 1) {
...
function startCafeGaugeLoop() {
  ...
  const speed = 1.8;
  function tick() {
    if (cafeStep > 1 || cafeGaugeLocked || cafeCompleted) { stopCafeGaugeLoop(); return; }
    ...
    if (m) m.style.left = cafeGaugePos + '%';
...
function tapCafeGauge() {
  if (cafeGaugeLocked || cafeCompleted) return;
  cafeGaugeLocked = true;
  stopCafeGaugeLoop();
  const pos = cafeGaugePos;
  let score, grade;
  if (pos >= 45.5 && pos <= 54.5) { score = 40; grade = 'PERFECT'; }
...
function renderBondList() {
  ...
  list.innerHTML = Object.values(CHARS).map(ch => {
    ...
    return `<div class="bond-card" onclick="${hasCard ? `openBondDetail('${ch.id}')` : ''}" style="${!hasCard ? 'opacity:0.4;cursor:default;' : ''}"><div class="bond-card-top"><div class="bond-char-placeholder" style="background:linear-gradient(135deg,${ch.gradeColor}88,${ch.gradeColor})">${ch.emoji}</div><div class="bond-info"><div class="bond-name">${ch.name}</div><div class="bond-heart-row"><div class="bond-hearts">${hearts}</div><div class="bond-level-text">${info.stage} Lv.${aff}</div></div><div class="bond-desc">${hasCard ? `인연 경험치 ${info.exp}` : '카드를 뽑아야 인연을 쌓을 수 있어'}</div></div></div></div>`;
  }).join('');
}
...
function openBondDetail(charId) {
  ...
  document.getElementById('affection-label').textContent = `${info.stage} Lv.${info.level} · ${info.exp} EXP`;
  document.getElementById('story-list').innerHTML = ch.stories.map(s => {
    ...
    return `<div class="story-item ${unlocked ? '' : 'locked'}" onclick="${unlocked ? `readStory('${charId}',${s.ep - 1})` : ''}"><div class="story-item-top"><div class="story-ep">EP.${s.ep}</div><div>${read ? '✅' : unlocked ? '📖' : '🔒'}</div></div><div class="story-title-text">${s.title}</div><div class="story-preview">${unlocked ? s.msgs[0].text.slice(0, 30) + '...' : '인연 Lv.' + s.unlockAt + ' 이상 필요'}</div></div>`;
  }).join('');
...
const PLACE_BUTTONS = {
  ...
  lake: ['btn-meet-ara'],
  room: ['btn-room-deco'],
  shopping: ['btn-shop-gift', 'btn-shop-room', 'btn-tech-academy'],
...
function giveGift(charId, item, fromBag) {
  ...
  const hearts = '❤️'.repeat(after.level) + '🤍'.repeat(5 - after.level);
  const affUp = before.stage !== after.stage || before.level !== after.level;
  const emoji = item.match(/[\u{1F300}-\u{1FFFF}]|[\u2600-\u26FF]/gu)?.[0] || '🎁';
  document.getElementById('meet-overlay').querySelector('div').innerHTML = `<div style="text-align:center;padding:8px 0;"><div style="font-size:32px;margin-bottom:8px;">${emoji}</div><div style="font-size:16px;font-weight:900;color:#fff;margin-bottom:6px;">${ch.name}에게 선물했어요!</div><div style="font-size:20px;margin-bottom:6px;">${hearts}</div><div style="font-size:13px;color:#FFD700;font-weight:700;">+${affExpGain} EXP · ${after.stage} Lv.${after.level}</div>${affUp ? '<div style="font-size:13px;color:#FF6B9D;font-weight:700;margin-top:4px;">💞 인연 단계 상승!</div>' : ''}${hintText}</div><button onclick="document.getElementById('meet-overlay').remove()" style="width:100%;margin-top:16px;padding:13px;background:linear-gradient(135deg,#FF6B9D,#C084FC);border:none;border-radius:12px;color:#fff;font-size:15px;font-weight:700;cursor:pointer;font-family:'Noto Sans KR',sans-serif;">확인</button>`;
}
...
function closeAttend() {
  ...
  if (day === 14) { stamina = 100; localStorage.setItem('ph_stamina', 100); }
...
function openTechAcademy() {
  ...
  const learned = learnedSkills.includes('sewing');
  const old = document.getElementById('tech-academy-overlay');
  if (old) old.remove();
  const overlay = document.createElement('div');
  overlay.id = 'tech-academy-overlay';
  overlay.style.cssText = 'position:fixed;inset:0;z-index:900;background:rgba(0,0,0,0.82);display:flex;align-items:center;justify-content:center;';
  overlay.innerHTML = `<div style="background:linear-gradient(135deg,#1a1a2e,#2d1b4e);border:2px solid #C084FC;border-radius:22px;padding:26px 22px;text-align:center;width:88%;max-width:320px;">
    <div style="font-size:44px;margin-bottom:8px;">🎓</div>
    <div style="font-size:20px;font-weight:900;color:#fff;margin-bottom:6px;">기술학원</div>
    <div style="font-size:13px;color:#aaa;line-height:1.6;margin-bottom:18px;">플레이어 Lv.10부터 입학할 수 있어요.<br>현재 Lv.${playerLevel}</div>
    ${!canEnter ? `<button onclick="document.getElementById('tech-academy-overlay').remove()" style="width:100%;padding:13px;background:rgba(255,255,255,0.08);border:none;border-radius:12px;color:#aaa;font-size:14px;font-weight:700;cursor:pointer;font-family:'Noto Sans KR',sans-serif;">아직 입학 불가</button>` : learned ? `<div style="font-size:14px;color:#4ade80;font-weight:900;margin-bottom:12px;">🧵 재봉 기술 습득 완료!</div><button onclick="document.getElementById('tech-academy-overlay').remove()" style="width:100%;padding:13px;background:rgba(255,255,255,0.08);border:none;border-radius:12px;color:#aaa;font-size:14px;font-weight:700;cursor:pointer;font-family:'Noto Sans KR',sans-serif;">닫기</button>` : `<button onclick="learnSewing()" style="width:100%;padding:14px;background:linear-gradient(135deg,#FF6B9D,#C084FC);border:none;border-radius:12px;color:#fff;font-size:15px;font-weight:900;cursor:pointer;font-family:'Noto Sans KR',sans-serif;">🧵 재봉 배우기</button>`}
  </div>`;
  document.body.appendChild(overlay);
}

function learnSewing() {
  if (playerLevel < 10) { showBagToast('🎓 레벨 10부터 재봉을 배울 수 있어요!'); return; }
  if (!learnedSkills.includes('sewing')) learnedSkills.push('sewing');
  localStorage.setItem('ph_learnedSkills', JSON.stringify(learnedSkills));
  saveAll();
  const overlay = document.getElementById('tech-academy-overlay');
  if (overlay) overlay.remove();
  showBagToast('🧵 재봉 기술을 배웠어요!');
}
...
// 카드 1장 = 경험치 1. 경험치 누적표는 기획 확정값 기준.
const AFFECTION_MILESTONES = [
  { exp: 0,    stage: '친절', level: 1 },
  { exp: 20,   stage: '친절', level: 2 },
  { exp: 50,   stage: '친절', level: 3 },
  { exp: 90,   stage: '친절', level: 4 },
  { exp: 150,  stage: '친절', level: 5 },
  { exp: 220,  stage: '우호', level: 1 },
  { exp: 320,  stage: '우호', level: 2 },
  { exp: 450,  stage: '우호', level: 3 },
  { exp: 600,  stage: '우호', level: 4 },
  { exp: 800,  stage: '우호', level: 5 },
  { exp: 1050, stage: '신뢰', level: 1 },
  { exp: 1350, stage: '신뢰', level: 2 },
  { exp: 1700, stage: '신뢰', level: 3 },
  { exp: 2100, stage: '신뢰', level: 4 },
  { exp: 2600, stage: '신뢰', level: 5 },
  { exp: 3200, stage: '인연', level: 1 },
  { exp: 4000, stage: '인연', level: 5 },
];

function getAffectionInfo(charId) {
  const exp = affectionExp[charId] || 0;
  let info = AFFECTION_MILESTONES[0];
  for (const m of AFFECTION_MILESTONES) {
    if (exp >= m.exp) info = m;
    else break;
  }
  return { stage: info.stage, level: info.level, exp };
}

function getAffectionLevel(charId) {
  return getAffectionInfo(charId).level;
}

function getAffectionTitle(charId) {
  const info = getAffectionInfo(charId);
  return `${info.stage} Lv.${info.level}`;
}

function hasMysteryIslandUnlock() {
  return Object.keys(CHARS).some(cid => (affectionExp[cid] || 0) >= 50); // 친절 Lv3
}

function syncAffectionLegacy(charId) {
  const info = getAffectionInfo(charId);
  affection[charId] = info.level;          // 기존 UI/스토리 호환용
  affectionLevel[charId] = info.level;     // 기존 퀘스트/잠금 호환용
}

function addAffectionExp(charId, amount) {
  if (!charId || !CHARS[charId]) return;
  const before = getAffectionInfo(charId);
  ...
  localStorage.setItem('ph_affectionLevel', JSON.stringify(affectionLevel));
  const after = getAffectionInfo(charId);
  if (before.stage !== after.stage || before.level !== after.level) {
    const ch = CHARS[charId];
    showBagToast(`💞 ${ch.name} ${after.stage} Lv.${after.level} 달성!`);
    if (after.stage === '친절' && after.level >= 2) checkQuestProgress('affection_level_2');
    if ((after.stage === '친절' && after.level >= 3) || hasMysteryIslandUnlock()) checkQuestProgress('mystery_island_unlock');
  }
}

Object.keys(CHARS).forEach(syncAffectionLegacy);
...
function startExplore(placeId) {
  // 신비의 섬 잠금 체크
  if (placeId === 'mystery') {
    if (!hasMysteryIslandUnlock()) {
      showBagToast('✨ 신비의 섬은 아무 포카나 친절 Lv.3부터 해금돼요!');
      return;
    }
  ...
  // 타이머
  const exploreDuration = 3000;
  const startTime = Date.now();
  exploreTimer = setInterval(() => {
    const elapsed = Date.now() - startTime;
    const remaining = Math.max(0, exploreDuration - elapsed);
    const pct = remaining / exploreDuration * 100;
...
function getEquippedStats() {
  ...
  const cloth = CLOTH_ITEMS.find(c => c.id === equippedCloth);
  if (!cloth) return {};
  return { [cloth.stat]: cloth.statVal };
}

function equipCloth(clothId) {
  equippedCloth = clothId;
  localStorage.setItem('ph_equippedCloth', JSON.stringify(clothId));
  const cloth = CLOTH_ITEMS.find(c => c.id === clothId);
  showBagToast(`👗 ${cloth.name} 착용 완료! ${cloth.desc}`);
  renderBag();
}
...
function openClothDetail(clothId) {
  const cloth = CLOTH_ITEMS.find(c => c.id === clothId);
  if (!cloth) return;
  const isOwned = bagItems.find(i => i.name === cloth.name);
  const isEquipped = equippedCloth === clothId;
  ...
  overlay.innerHTML = `<div style="background:linear-gradient(135deg,#1a1a2e,#2d1b4e);border:2px solid #C084FC;border-radius:20px;padding:20px;text-align:center;width:85%;max-width:300px;">
    <img src="${cloth.img}" style="width:100%;border-radius:12px;margin-bottom:12px;object-fit:cover;max-height:200px;">
    <div style="font-size:16px;font-weight:900;color:#fff;margin-bottom:4px;">${cloth.name}</div>
    <div style="font-size:12px;color:#C084FC;margin-bottom:4px;">${cloth.grade}</div>
    <div style="font-size:13px;color:#FFD700;margin-bottom:16px;">${cloth.desc}</div>
    ${isOwned ? (isEquipped ?
