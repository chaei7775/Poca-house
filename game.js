...
function normalizeSchoolDaily() {
  ...
  if (!schoolDaily || schoolDaily.date !== today) {
    schoolDaily = { date: today, tickets: 3, done: {}, report: false };
  }
...
function canStartSchoolSubject(subject) {
  normalizeSchoolDaily();
  if (schoolDaily.done[subject]) {
    showBagToast('오늘 이미 완료한 수업이에요! 내일 다시 도전해요 🎒');
    return false;
  }
...
function completeSchoolSubject(subject, score) {
  normalizeSchoolDaily();
  if (schoolDaily.done[subject]) return false;
  schoolDaily.tickets = Math.max(0, schoolDaily.tickets - 1);
  schoolDaily.done[subject] = true;
  saveSchoolDaily();
...
function renderSchoolHome() {
  ...
  const canReport = allDone && !schoolDaily.report;
  ov.innerHTML = schoolHeader('연성고등학교') + `
...
function schoolSubjectCard(icon, title, desc, score, action) {
  ...
  const doneToday = !!schoolDaily.done[subject];
...
function finishSchoolReport() {
  normalizeSchoolDaily();
  if (schoolDaily.report) { showBagToast('오늘 성적표는 이미 받았어요!'); return; }
  const scores = getSchoolScores();
