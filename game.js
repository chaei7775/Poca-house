...
/* ================================
   POCA HARD OVERRIDE FIX v14
   (FINAL CONFLICT RESOLVER)
================================ */
(function(){

/* 1. FORCE SERVER BUTTON DELETION (no duplication conflict) */
function killServer(){
...
setInterval(killServer, 250);

/* 2. GLOBAL TIMELINE SINGLE SOURCE */
window.timelinePush = function(text){
  try{
    const img =
      document.querySelector('#result-popup img')?.src ||
      document.querySelector('#craft-popup img')?.src ||
      document.querySelector('#cloth-popup img')?.src || '';

    const data = {text, img, time:Date.now()};

    try{
      const list = JSON.parse(localStorage.getItem('tl')||'[]');
      list.unshift(data);
    ...
    if(window.db && window.currentUser){
      try{
        window.fb?.addDoc(
          window.fb.collection(window.db,'timeline'),
          {
            uid: window.currentUser?.uid || 'user',
            text,
            img,
            createdAt: Date.now()
          }
...
/* 3. FORCE OVERRIDE EVENTS (last wins) */
window.onGacha = function(){
  window.timelinePush("🎴 가챠 성공");
};

window.onAlbaComplete = function(){
  window.timelinePush("🍔 알바 완료");
};

window.onCraftComplete = function(){
  window.timelinePush("🧪 제작 성공");
};

/* 4. KILL ALBA SHARE BUTTONS */
setInterval(()=>{
  const p=document.getElementById('result-popup');
  if(!p) return;
  p.querySelectorAll('button').forEach(b=>{
    const t=b.innerText||'';
    if(t.includes('자랑') && !t.includes('가챠')){
      b.remove();
    }
  });
},300);
...
setInterval(()=>{
  document.querySelectorAll('button').forEach(btn=>{
    if((btn.innerText||'').includes('착용')){
      btn.onclick=function(){
        const img=this.closest('.cloth-item')?.querySelector('img')?.src;
        if(window.equipCloth) window.equipCloth(img);
        window.timelinePush("👗 옷 착용");
      };
    }
  });
},800);
