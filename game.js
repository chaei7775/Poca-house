...
function startFlowerJob(levelKey) {
  ...
  flowerState = { levelKey, bouquet: rec.bouquet, sequence: rec.seq, input: [], mistakes: 0, startedAt: Date.now(), phase:'preview', completed:false };
  renderFlowerPlay();
  playFlowerPreview();
}
function renderFlowerPlay() {
  ...
  const progress = flowerState.input.map(f => `${f.emoji}`).join(' ');
  const targetPreview = flowerState.phase === 'preview' ? flowerState.sequence.map(f => `<span style="display:inline-flex;align-items:center;justify-content:center;width:36px;height:36px;border-radius:12px;background:#fff;color:#111;margin:2px;font-size:20px;border:2px solid ${cfg.color};">${f.emoji}</span>`).join('') : '<span style="color:#FFB3CC;font-weight:900;">순서대로 꽃을 선택하세요!</span>';
  const buttons = FLOWER_ITEMS.map((f, i) => `<button id="flower-btn-${i}" class="flower-choice-btn" onclick="tapFlower('${f.id}')" ${flowerState.phase !== 'input' ? 'disabled' : ''} style="min-height:42px;border:1.5px solid ${flowerState.phase === 'input' ? '#fbcfe8' : '#555'};border-radius:12px;background:${flowerState.phase === 'input' ? 'rgba(255,255,255,0.96)' : 'rgba(255,255,255,0.3)'};color:#111;font-size:11px;font-weight:900;font-family:'Noto Sans KR',sans-serif;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:1px;cursor:${flowerState.phase === 'input' ? 'pointer' : 'default'};"><span style="font-size:18px;line-height:1;">${f.emoji}</span><span style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100%;">${f.name}</span></button>`).join('');
  panel.innerHTML = `<div style="color:#fff;"><div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:7px;"><div style="font-size:13px;font-weight:1000;color:${cfg.color};">${cfg.label} 주문</div><div style="font-size:12px;color:#FFD700;font-weight:900;">실수 ${flowerState.mistakes}/3</div></div><div style="background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.14);border-radius:14px;padding:9px;text-align:center;margin-bottom:8px;"><div style="font-size:13px;font-weight:1000;margin-bottom:4px;">💐 ${flowerState.bouquet}</div><div style="min-height:40px;">${targetPreview}</div><div style="font-size:11px;color:#aaa;margin-top:5px;">내 선택: ${progress || '-'}</div></div><div style="display:grid;grid-template-columns:repeat(6,1fr);gap:5px;">${buttons}</div><button onclick="initFlowerAlba()" style="margin-top:8px;width:100%;padding:10px;background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.2);border-radius:12px;color:#fff;font-weight:900;font-family:'Noto Sans KR',sans-serif;">주문 취소</button></div>`;
  if (hot) hot.innerHTML = flowerState.phase === 'preview' ? `<div style="position:absolute;left:50%;top:12%;transform:translateX(-50%);background:rgba(26,26,46,0.78);border:2px solid #FFD700;border-radius:18px;padding:8px 14px;color:#fff;font-size:13px;font-weight:1000;box-shadow:0 0 20px #FFD70066;">3초 동안 순서 확인!</div>` : '';
}
function playFlowerPreview() {
  ...
  flowerState.sequence.forEach((flower, idx) => {
    const t = setTimeout(() => {
      const index = FLOWER_ITEMS.findIndex(f => f.id === flower.id);
      const btn = document.getElementById('flower-btn-' + index);
      if (btn) {
        btn.style.boxShadow = '0 0 0 4px #FFD700, 0 0 24px #FFD700';
        btn.style.transform = 'scale(1.08)';
        setTimeout(() => { if (btn) { btn.style.boxShadow=''; btn.style.transform=''; } }, 420);
      }
    }, 450 + idx * 430);
