...
function schoolHeader(title) {
  return `<div style="position:sticky;top:0;z-index:2;background:linear-gradient(135deg,#60a5fa,#C084FC);padding:14px 16px;color:#fff;box-shadow:0 2px 12px #0002;display:flex;align-items:center;justify-content:space-between;gap:8px;">
    ...
    <div style="font-size:12px;font-weight:900;background:rgba(255,255,255,0.22);border-radius:14px;padding:6px 9px;white-space:nowrap;">⭐ ${honorStars}</div>
  </div>`;
}
    ...
    <div style="padding:18px 16px 120px;display:flex;flex-direction:column;gap:14px;">
      <div style="background:#fff;border:2px solid #bfdbfe;border-radius:20px;padding:14px;box-shadow:0 4px 18px #60a5fa22;display:flex;align-items:center;gap:12px;">
        ...
        <div style="flex:1;">
          ...
          <div style="font-size:12px;color:#64748b;line-height:1.45;margin-top:3px;">세 과목을 끝내면 성적표와 우등생별을 받아요.</div>
        </div>
      ...
      <button onclick="finishSchoolReport()" ${allDone ? '' : 'disabled'} style="width:100%;padding:16px;border:none;border-radius:16px;font-size:16px;font-weight:900;font-family:'Noto Sans KR',sans-serif;cursor:${allDone ? 'pointer' : 'not-allowed'};background:${allDone ? 'linear-gradient(135deg,#F59E0B,#FF6B9D)' : '#ddd'};color:${allDone ? '#fff' : '#999'};box-shadow:${allDone ? '0 4px 18px #F59E0B44' : 'none'};">📋 성적표 받기</button>
      <button onclick="resetSchoolScores()" style="width:100%;padding:12px;border:none;border-radius:14px;background:rgba(0,0,0,0.06);color:#555;font-weight:900;font-family:'Noto Sans KR',sans-serif;cursor:pointer;">🔄 오늘 점수 다시 하기</button>
    </div>`;
  ov.style.display = 'block';
}
...
function schoolSubjectCard(icon, title, desc, score, action) {
  const scoreText = typeof score === 'number' ? `${score}점` : '미완료';
  return `<button onclick="${action}" style="text-align:left;width:100%;padding:16px;background:#fff;border:2px solid #fbcfe8;border-radius:18px;box-shadow:0 4px 16px #FF6B9D18;display:flex;align-items:center;gap:12px;font-family:'Noto Sans KR',sans-serif;cursor:pointer;">
    <div style="font-size:34px;width:48px;text-align:center;">${icon}</div>
    <div style="flex:1;"><div style="font-size:15px;font-weight:900;color:#111;">${title}</div><div style="font-size:12px;color:#777;margin-top:3px;">${desc}</div></div>
    <div style="font-size:13px;font-weight:900;color:${typeof score === 'number' ? '#F59E0B' : '#94a3b8'};">${scoreText}</div>
  </button>`;
}
...
function finishSchoolReport() {
  ...
  let rank = 'C', reward = 1;
  if (avg >= 90) { rank = 'S'; reward = 4; }
  else if (avg >= 80) { rank = 'A'; reward = 3; }
  else if (avg >= 70) { rank = 'B'; reward = 2; }
  honorStars += reward;
  addPlayerExp(8);
  clearSchoolScoresForSelected();
  const ov = ensureSchoolOverlay();
  ov.innerHTML = schoolHeader('📋 성적표') + `
    <div style="padding:28px 16px 120px;text-align:center;">
      <div style="background:#fff;border:2px solid #fde68a;border-radius:24px;padding:26px 18px;box-shadow:0 6px 24px #F59E0B22;">
        ...
        <div style="font-size:18px;font-weight:900;color:#111;margin:12px 0;">우등생별 +${reward} ⭐</div>
        <div style="font-size:12px;color:#777;line-height:1.5;margin-bottom:20px;">국어 ${vals[0]} · 암기카드 ${vals[1]} · 체육 ${vals[2]}<br>나중에 디자이너샵에서 포카 스킨을 살 수 있어요.</div>
