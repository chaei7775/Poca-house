...
function findClothBagItem(clothId, itemName) {
  if (typeof normalizeClothBagItems === 'function') normalizeClothBagItems();
  const cloth = CLOTH_ITEMS.find(c => c.id === clothId);
  ...
  return bagItems.find(i => {
    ...
    const itemClean = (i.name || '').replace(/^✨\s*/, '').trim();
    return (
      i.clothId === clothId ||
      i.name === itemName ||
      itemClean === cleanName ||
      i.name === cloth.name ||
      itemClean === cloth.name
    );
  }) || null;
}
...
function equipCloth(clothId, itemName) {
  ...
  const cloth = CLOTH_ITEMS.find(c => c.id === clothId);
  if (!cloth) {
    showBagToast('의상 정보를 찾지 못했어요!');
    return;
  }

  const item = findClothBagItem(clothId, itemName);
  if (!item) {
    showBagToast('보유한 의상이 아니에요!');
    return;
  }
  ...
  equippedCloth = {
    ...
    name: item.name,
    stat: cloth.stat,
    statVal: item.statVal || cloth.statVal,
...
function unequipCloth() {
  ...
  saveAll();
  if (typeof renderBag === 'function') renderBag();
  if (typeof schedulePocaServerSave === 'function') schedulePocaServerSave('unequipCloth');
...
function showServerSyncToast(msg, ok) {
  ...
  el.style.cssText = `position:fixed;left:50%;top:4px;transform:translateX(-50%);z-index:9999;background:${ok ? 'rgba(22,163,74,.88)' : 'rgba(220,38,38,.88)'};color:#fff;border-radius:999px;padding:3px 8px;font-size:10px;font-weight:900;box-shadow:0 2px 8px #0003;max-width:70vw;text-align:center;pointer-events:none;`;
...
function addFirebaseTestPanel() {
  ...
  if (old) old.remove();
  // 수동 서버저장 버튼은 탐험 재료 클릭과 겹치므로 만들지 않음.
  // 자동 저장 + 상단 1초 토스트만 사용.
}
...
/* ===== POCA HOUSE SERVER + CLOTH SAFETY PATCH ===== */
(function(){
  function removeFirebaseTestPanel(){
    const panel = document.getElementById('firebase-test-panel');
    if (panel) panel.remove();
  }
  removeFirebaseTestPanel();
  setTimeout(removeFirebaseTestPanel, 500);
  setTimeout(removeFirebaseTestPanel, 2000);

  try {
    const saved = localStorage.getItem('ph_equippedCloth');
    if (saved && saved !== 'null') {
      const parsed = JSON.parse(saved);
      if (parsed && typeof parsed === 'object') equippedCloth = parsed;
    }
  } catch(e) {}
})();
