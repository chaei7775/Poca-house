...
const FLOWER_LEVELS = {
  easy: { label:'초급', count:4, rewards:{ FAIL:80, GOOD:150, GREAT:220, PERFECT:300 }, color:'#22c55e' },
  normal: { label:'중급', count:6, rewards:{ FAIL:120, GOOD:250, GREAT:380, PERFECT:500 }, color:'#F59E0B' },
  hard: { label:'고급', count:8, rewards:{ FAIL:200, GOOD:400, GREAT:650, PERFECT:900 }, color:'#FF6B9D' }
};
...
function renderFlowerMenu() {
  ...
  if (!panel) return;
  panel.innerHTML = `<div style="color:#fff;text-align:center;"><div style="font-size:18px;font-weight:1000;margin-bottom:4px;">🌷 꽃다발 주문 난이도</div><div style="font-size:12px;color:#FFB3CC;line-height:1.5;margin-bottom:12px;">순서대로 반짝이는 꽃 위치를 기억하고 직접 눌러요.</div><div style="display:grid;grid-template-columns:1fr;gap:8px;"><button onclick="startFlowerJob('easy')" style="padding:13px;border:none;border-radius:14px;background:linear-gradient(135deg,#22c55e,#86efac);color:#fff;font-weight:1000;font-size:15px;font-family:'Noto Sans KR',sans-serif;">🌱 초급 · 꽃 4개 · 최대 300코인</button><button onclick="startFlowerJob('normal')" style="padding:13px;border:none;border-radius:14px;background:linear-gradient(135deg,#F59E0B,#FDE68A);color:#5b2a00;font-weight:1000;font-size:15px;font-family:'Noto Sans KR',sans-serif;">🌼 중급 · 꽃 6개 · 최대 500코인</button><button onclick="startFlowerJob('hard')" style="padding:13px;border:none;border-radius:14px;background:linear-gradient(135deg,#FF6B9D,#C084FC);color:#fff;font-weight:1000;font-size:15px;font-family:'Noto Sans KR',sans-serif;">💐 고급 · 꽃 8개 · 최대 900코인</button></div></div>`;
}
