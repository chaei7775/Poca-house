...
/* ===== POCA EMERGENCY FIX v7 ===== */
(function(){

  /* =========================
     1. FORCE REMOVE SERVER BUTTON
  ==========================*/
  function removeServerBtn(){
    try{
      const all = document.querySelectorAll('button, div');
      all.forEach(el=>{
        const t = (el.innerText || el.textContent || '').trim();
        if(t.includes('서버저장') || t.includes('저장')) {
          // 노란 버튼일 가능성 높은 것만 제거 (상단 toast 제외)
          if(el.id !== 'server-sync-toast'){
            el.remove();
          }
        }
      });
  ...
  setInterval(removeServerBtn, 500);

  /* =========================
     2. TIMELINE STORAGE (UI ALSO)
  ==========================*/
  ...
  function createTimelineUI(){
    ...
    box.id = 'timeline-ui';
    box.style.cssText =
      'position:fixed;left:10px;bottom:10px;width:160px;height:220px;' +
      'background:rgba(0,0,0,.7);color:#fff;z-index:99998;' +
      'font-size:10px;overflow:auto;padding:6px;border-radius:10px;';

    document.body.appendChild(box);
  }

  function renderTimeline(){
    const box = document.getElementById('timeline-ui');
    if(!box) return;

    const list = getTimeline().slice(0,10);
    box.innerHTML = '<b>📌 TIMELINE</b><br/>';

    list.forEach(v=>{
      box.innerHTML += `
        <div style="margin-top:6px;padding:4px;border-bottom:1px solid #333;">
          <div>${v.text || ''}</div>
          ${v.img ? `<img src="${v.img}" style="width:100%;border-radius:6px;margin-top:3px;">` : ''}
        </div>
      `;
    });
  }
