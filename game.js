...
function schoolHeader(title) {
  normalizeSchoolDaily();
  return `<div style="position:sticky;top:0;z-index:2;background:linear-gradient(135deg,#60a5fa,#C084FC);padding:12px 14px;color:#fff;box-shadow:0 2px 12px #0002;display:flex;align-items:center;justify-content:space-between;gap:8px;">
    <button onclick="closeSchoolGames()" style="background:rgba(0,0,0,0.22);border:none;border-radius:10px;color:#fff;padding:8px 11px;font-weight:900;cursor:pointer;">←</button>
    <div style="font-size:17px;font-weight:900;flex:1;text-align:center;">${title}</div>
    <div style="font-size:11px;font-weight:900;background:rgba(255,255,255,0.22);border-radius:14px;padding:5px 8px;white-space:nowrap;line-height:1.35;text-align:right;">🎫 ${schoolDaily.tickets}/3<br>⭐ ${honorStars} · ✨ ${honorFragments}</div>
  </div>`;
}
...
function renderSchoolHome() {
  ...
  ov.innerHTML = schoolHeader('연성고등학교') + `
    <div style="padding:18px 16px 120px;display:flex;flex-direction:column;gap:14px;">
      <div style="background:#fff;border:2px solid #bfdbfe;border-radius:20px;padding:14px;box-shadow:0 4px 18px #60a5fa22;display:flex;align-items:center;gap:12px;">
        <img class="school-card-img" src="${card.img}" style="width:44px;height:58px;object-fit:cover;border-radius:8px;background:#eee;flex-shrink:0;" onerror="this.style.display='none'">
        <div style="flex:1;">
          <div style="font-size:18px;font-weight:900;color:#1d4ed8;">🎒 ${card.name} 등교 중</div>
          <div style="font-size:12px;color:#64748b;line-height:1.45;margin-top:3px;">수업 1회마다 🎫 1장 사용 · 성적표에서 별/조각을 받아요.</div>
        </div>
        <button onclick="renderSchoolSelect()" style="border:none;border-radius:12px;background:#eff6ff;color:#1d4ed8;font-weight:900;padding:9px 10px;cursor:pointer;font-family:'Noto Sans KR',sans-serif;">바꾸기</button>
      </div>
...
function schoolSubjectCard(icon, title, desc, score, action) {
  ...
  return `<button onclick="${disabled ? '' : action}" ${disabled ? 'disabled' : ''} style="text-align:left;width:100%;padding:16px;background:${doneToday ? '#f8fafc' : '#fff'};border:2px solid ${doneToday ? '#cbd5e1' : '#fbcfe8'};border-radius:18px;box-shadow:0 4px 16px #FF6B9D18;display:flex;align-items:center;gap:12px;font-family:'Noto Sans KR',sans-serif;cursor:${disabled ? 'not-allowed' : 'pointer'};opacity:${disabled && !doneToday ? '0.62' : '1'};">
    <div style="font-size:34px;width:48px;text-align:center;">${icon}</div>
    <div style="flex:1;"><div style="font-size:15px;font-weight:900;color:#111;">${title}</div><div style="font-size:12px;color:#777;margin-top:3px;">${desc}<br>🎫 등교티켓 1장 필요</div></div>
    <div style="font-size:13px;font-weight:900;color:${typeof score === 'number' ? '#F59E0B' : doneToday ? '#64748b' : '#94a3b8'};">${scoreText}</div>
  </button>`;
}
