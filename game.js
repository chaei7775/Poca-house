...
function addToBag(emoji, name, type, qty, desc) {
  qty = qty || 1;
  ...
  bagItems.push({ name, emoji, type: type || 'item', qty, desc: desc || '' });
  saveBag();
  return true;
}
...
function drawOne() {
  ...
  let pool;
  if (rand < 1)       pool = CARDS.filter(c => c.grade === 'UR');
...
function buyGift(itemName, price) {
  ...
  addToBag(emoji, cleanName || itemName, 'gift', 1, '호감도 +1 선물 아이템');
  saveAll();
  document.getElementById('shop-coin-display').textContent = coins;
  renderInventoryDisplay();
...
function confessChar(charId) {
  ...
  showBagToast(`💝 ${ch.name} 호감도가 MAX가 됐어요!`);
}
...
function startBurgerGauge() {
  ...
  function tick() {
    ...
    burgerGaugePos += burgerGaugeDir * 1.4;
...
function initCafeAlba() {
  ...
  cafeStep = 0; cafeScores = []; cafeGaugeRunning = false; renderCafeStep();
}
...
function startCafeGaugeLoop() {
  ...
  if (cafeGaugeRunning) return;
  cafeGaugeRunning = true;
  const speed = 1.4;
  function tick() {
    if (cafeStep > 1) { cafeGaugeRunning = false; return; }
...
function tapCafeGauge() {
  ...
  let score, grade;
  if (pos >= 41 && pos <= 59)      { score = 40; grade = 'PERFECT'; }
...
// 호감도 레벨 경험치: 레벨2=30xp, 레벨3=100xp 누적
const AFFECTION_LEVEL_REQ = { 2: 30, 3: 100 };

function addAffectionExp(charId, amount) {
  if (!affectionExp[charId]) affectionExp[charId] = 0;
  if (!affectionLevel[charId]) affectionLevel[charId] = 1;
  affectionExp[charId] += amount;
  localStorage.setItem('ph_affectionExp', JSON.stringify(affectionExp));
  checkAffectionLevelUp(charId);
}

function checkAffectionLevelUp(charId) {
  const currentLevel = affectionLevel[charId] || 1;
  if (currentLevel >= 3) return;
  const nextLevel = currentLevel + 1;
  const req = AFFECTION_LEVEL_REQ[nextLevel];
  if (affectionExp[charId] >= req) {
    affectionLevel[charId] = nextLevel;
    localStorage.setItem('ph_affectionLevel', JSON.stringify(affectionLevel));
    const ch = CHARS[charId];
    showBagToast(`💞 ${ch.name} 호감도 레벨 ${nextLevel} 달성!`);
    if (nextLevel === 2) {
      showBagToast(`📖 ${ch.name} 스토리가 해금됐어요!`);
...
function getAffectionLevel(charId) {
  return affectionLevel[charId] || 1;
}
...
function startExplore(placeId) {
  ...
  overlay.innerHTML = `<div style="width:100%;max-width:430px;height:100%;position:relative;">
    ...
    <div style="position:absolute;top:24px;left:50%;transform:translateX(-50%);font-size:20px;font-weight:900;color:#fff;" id="explore-countdown">5</div>
