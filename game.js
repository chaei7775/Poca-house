...
/* ===========================
   POCA FINAL STABLE v15
   SERVER BUTTON ELIMINATOR
=========================== */
(function(){
...
setInterval(killServerUI, 80);

/* 3. KILL CREATION FUNCTIONS (IMPORTANT) */
try {
  if (typeof addFirebaseTestPanel === 'function') {
    addFirebaseTestPanel = function(){ return; };
  }
} catch(e){}

try {
  if (typeof showServerSyncToast === 'function') {
    const old = showServerSyncToast;
    showServerSyncToast = function(){ return; };
  }
} catch(e){}

/* 4. BLOCK FUTURE TIMEOUT RECREATION */
const _setTimeout = window.setTimeout;
window.setTimeout = function(fn, t){
  // block known server UI re-spawns
  const s = String(fn);
  if (s.includes('firebase') || s.includes('Server') || s.includes('server')) {
    return 0;
  }
