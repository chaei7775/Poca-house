...
function renderFlowerPlay() {
  ...
  const targetPreview = flowerState.phase === 'preview'
    ? `<div style="font-size:11px;color:#FFD700;font-weight:900;margin-bottom:4px;">5초 동안 제작순서 확인!</div><div>${flowerOrderIcons(flowerState.sequence, true)}</div>`
    : `<div style="font-size:12px;color:#FFB3CC;font-weight:900;margin-bottom:4px;">이미지 속 꽃을 순서대로 터치!</div><div style="min-height:34px;">${flowerOrderIcons(flowerState.input, false) || '<span style="font-size:12px;color:#aaa;">아직 선택 없음</span>'}</div>`;
