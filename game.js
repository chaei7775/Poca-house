...
/* ===== CLOTH SYSTEM UNIFY PATCH v1 ===== */

(function(){

window.isEquippedCloth = function(item){
  if (!item || !window.equippedCloth) return false;
  return window.equippedCloth.name === item.name || window.equippedCloth.clothId === item.clothId;
};

window.toggleCloth = function(clothId, itemName, idx){
  try{
    const item = bagItems[idx];
    const cloth = (typeof CLOTH_ITEMS !== 'undefined')
      ? CLOTH_ITEMS.find(c => c.id === clothId || c.name === itemName)
      : null;

    if(!cloth) return;

    if (window.equippedCloth && (window.equippedCloth.clothId === cloth.id || window.equippedCloth.name === cloth.name)){
      if (typeof unequipCloth === 'function') unequipCloth();
      return;
    }

    if (typeof equipCloth === 'function'){
      equipCloth(cloth.id, itemName);
    }
