...
(function(){
  ...
  if (window.showServerSyncToast) {
    const old = window.showServerSyncToast;
    window.showServerSyncToast = function(msg, ok){
      const el = document.getElementById('server-sync-toast');
      if (el) el.remove();
  ...
  /* 2. 알바: 자랑하기 버튼 제거 (result-popup 내부에서 삭제) */
  function removeAlbaShareBtn(){
    const p = document.getElementById('result-popup');
    if (!p) return;
    const btns = p.querySelectorAll('button');
    btns.forEach(b=>{
      if ((b.textContent || '').includes('자랑')) b.remove();
    });
  }

  /* 3. 가챠/성적/제작 팝업 5초 유지 */
  function keepPopup(selector){
    setTimeout(()=>{
  ...
  if (window.showGachaResult){
    ...
    window.showGachaResult = function(){
      const r = old.apply(this, arguments);
      keepPopup('#gacha-overlay');
      keepPopup('#gacha-single-fancy-overlay');
      return r;
    };
  }
  ...
  if (window.showResult){
    ...
    window.showResult = function(){
      const r = old.apply(this, arguments);
      keepPopup('#result-popup');
      removeAlbaShareBtn();
      return r;
    };
  }

  if (window.showCraftResult){
    const old = window.showCraftResult;
    window.showCraftResult = function(){
      const r = old.apply(this, arguments);
      keepPopup('#craft-popup');
      keepPopup('#craft-result-popup');
      return r;
    };
  }

  /* 4. 소원의 조각/결정: 5초 유지 */
  if (window.showWishFragment){
    const old = window.showWishFragment;
    window.showWishFragment = function(){
      const r = old.apply(this, arguments);
      keepPopup('#wish-popup');
      keepPopup('#item-popup');
      return r;
    };
  }

  if (window.showWishCrystal){
    const old = window.showWishCrystal;
    window.showWishCrystal = function(){
      const r = old.apply(this, arguments);
      keepPopup('#wish-popup');
      keepPopup('#item-popup');
      return r;
  ...
  /* 5. 옷 착용 버튼 복구 (최소 안전 래핑) */
  if (window.equipCloth){
    const old = window.equipCloth;
    window.equipCloth = function(){
      try{
        return old.apply(this, arguments);
      }catch(e){
        console.warn('equipCloth fallback', e);
      }
    };
  }
