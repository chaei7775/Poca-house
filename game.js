...
/* =========================================================
   POCA HOUSE FULL SOCIAL PATCH v3 (UNIFIED)
   - server save toast fix
   - cloth equip fix
   - universal share system (ALL EVENTS)
========================================================= */

/* ---------------------------
   🔔 SERVER SAVE TOAST
----------------------------*/
window.showServerSave = function(){
  const el = document.createElement("div");
  el.innerText = "✔ 저장";
...
window.equipCloth = function(item){
  window.equippedCloth = item;

  try{
    localStorage.setItem("ph_equippedCloth", JSON.stringify(item));
  }catch(e){}
...
try{
  ...
  if(saved) window.equippedCloth = JSON.parse(saved);
}catch(e){}

/* ---------------------------
   📢 UNIVERSAL SHARE SYSTEM
   (ALL EVENTS: alba, gacha, explore, school, craft)
----------------------------*/

window.shareEvent = function(type, payload){
  const overlay = document.createElement("div");
  ...
  const text = payload?.text || "공유";

  overlay.innerHTML = `
    <div style="width:85%;max-width:320px;background:#1a1a2e;border-radius:16px;padding:16px;text-align:center;">
      <div style="font-size:16px;font-weight:800;margin-bottom:10px;">📢 자랑하기</div>

      ${img ? `<img src="${img}" style="width:100%;border-radius:12px;margin-bottom:10px;">` : ""}

      <div style="font-size:14px;margin-bottom:12px;">${text}</div>

      <button id="sharePostBtn" style="width:100%;padding:12px;border:none;border-radius:10px;background:#ff4d8d;color:#fff;font-weight:800;">
        타임라인 올리기
      </button>

      <button onclick="this.closest('div[style*=fixed]').remove()" style="margin-top:8px;width:100%;padding:10px;border:none;border-radius:10px;background:#333;color:#aaa;">
        닫기
      </button>
    </div>
  `;
  ...
  document.getElementById("sharePostBtn").onclick = function(){
    // safe fallback (no dependency)
    if(window.postTimeline){
      window.postTimeline(`[${type}] ${text}`);
    }
    overlay.remove();
  };
};
...
/* ---------------------------
   🎮 AUTO HOOK HELPERS
   (attach these to your game events)
----------------------------*/

window.onAlbaComplete = function(result){
  window.shareEvent("alba", {
    text: result || "알바 완료!",
    image: window.lastAlbaImage || ""
  });
};

window.onGacha = function(card){
  window.shareEvent("gacha", {
    text: card?.name || "카드 획득!",
    image: card?.img || ""
  });
};

window.onExplore = function(item){
  window.shareEvent("explore", {
    text: item?.name || "탐험 성공!",
    image: item?.img || ""
  });
};

window.onSchoolResult = function(scoreImg){
  window.shareEvent("school", {
    text: "성적표 공개!",
    image: scoreImg || ""
  });
};

window.onCraftResult = function(item){
  window.shareEvent("craft", {
    text: "제작 완료!",
    image: item?.img || ""
  });
};
