...
function startBurgerGauge() {
  ...
  burgerGaugePos = 0;
  burgerGaugeDir = 1;
  burgerGaugeLocked = false;

  let lastTs = 0;
  const speedPerSec = 78; // 프레임 속도와 무관하게 일정한 속도

  function tick(ts) {
    const m = document.getElementById('burger-gauge-marker');
    if (!m || burgerGaugeLocked) { stopBurgerGauge(); return; }

    if (!lastTs) lastTs = ts || performance.now();
    const now = ts || performance.now();
    const dt = Math.min(34, Math.max(0, now - lastTs));
    lastTs = now;

    burgerGaugePos += burgerGaugeDir * speedPerSec * (dt / 1000);
...
function startCafeGaugeLoop() {
  ...
  function tick(ts) {
    if (cafeStep > 1 || cafeGaugeLocked) { stopCafeGauge(); return; }
    ...
    cafeGaugePos += cafeGaugeDir * speedPerSec * (dt / 1000);
...
function renderFlowerHotspots() {
  const hot = document.getElementById('flower-hotspots');
  if (!hot) return;
  if (!flowerState || flowerState.phase !== 'input') {
  ...
  hot.style.touchAction = 'none';
  hot.innerHTML = FLOWER_ITEMS.map(f => {
    const p = FLOWER_POS[f.id];
    if (!p) return '';
    return `<button class="flower-hotspot" data-flower-id="${f.id}" onpointerdown="event.preventDefault(); tapFlower('${f.id}');" onclick="event.preventDefault();" aria-label="${f.name}" style="position:absolute;left:${p.x}%;top:${p.y}%;transform:translate(-50%,-50%);width:19%;height:11%;border:2px solid transparent;border-radius:20px;background:rgba(255,255,255,0.001);cursor:pointer;z-index:50;touch-action:none;-webkit-tap-highlight-color:transparent;"></button>`;
  }).join('');
}
...
let flowerLastTapAt = 0;
let flowerLastTapId = '';
function tapFlower(flowerId) {
  const now = Date.now();
  if (flowerLastTapId === flowerId && now - flowerLastTapAt < 120) return;
  flowerLastTapAt = now;
  flowerLastTapId = flowerId;
  ...
  if (flower.id !== target.id) {
    ...
    if (flowerState.mistakes >= 3) {
      flowerState.input.push(flower);
      return finishFlowerJob();
    }
  } else {
    flowerState.combo++;
...
function showServerSyncToast(msg, ok) {
  ...
  el.style.cssText = `position:fixed;left:50%;top:4px;transform:translateX(-50%);z-index:1400;background:${ok ? 'rgba(22,163,74,.88)' : 'rgba(220,38,38,.88)'};color:#fff;border-radius:999px;padding:3px 8px;font-size:10px;font-weight:900;box-shadow:0 2px 8px #0003;max-width:72vw;text-align:center;pointer-events:none;`;
  el.textContent = ok ? '✔ 저장' : '저장 실패';
  document.body.appendChild(el);
...
function addFirebaseTestPanel() {
  const old = document.getElementById('firebase-test-panel');
  if (old) old.remove();
  // 수동 서버저장 패널은 게임 화면을 가려서 생성하지 않음.
}
...
/* ===== POCA PLAYABILITY HOTFIX =====
   replacements: toast=1, panel=1, burgerGauge=1, cafeGauge=1, flowerHotspots=1, flowerTap=1
*/
(function(){
  // 서버저장 패널이 혹시 남아있으면 계속 제거. 저장 토스트는 유지.
  function removeServerPanelOnly(){
    const p = document.getElementById('firebase-test-panel');
    if (p) p.remove();
  }
  removeServerPanelOnly();
  setInterval(removeServerPanelOnly, 1000);

  // 옷 착용 함수 전역 노출 보강
  try { if (typeof equipCloth === 'function') window.equipCloth = equipCloth; } catch(e) {}
  try { if (typeof openClothDetail === 'function') window.openClothDetail = openClothDetail; } catch(e) {}
  try { if (typeof unequipCloth === 'function') window.unequipCloth = unequipCloth; } catch(e) {}
})();
