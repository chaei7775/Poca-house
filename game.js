...
function renderCafeStep() {
  ...
  if (cafeStep === 0) {
    ...
    const espressoPerfect = cafeScores[0] === 40;
    const milkPerfect = cafeScores[1] === 40;
    const artPerfect = cafeScores[2] === 40;
    let comboBonus = 0;
    let comboText = '';
    if (espressoPerfect && milkPerfect && artPerfect) {
      comboBonus = 50;
      comboText = '🔥 3콤보 보너스 +50';
    } else if (espressoPerfect && milkPerfect) {
      comboBonus = 20;
      comboText = '🔥 2콤보 보너스 +20';
    }
    const rawBonus = baseBonus + comboBonus;
    if (!cafeCompleted) {
      cafeCompleted = true;
      cafeLastBonus = applyClothCoinBonus(applyAlbaDifficultyReward(rawBonus, cafeDifficulty, 'COMPLETE'));
    ...
    const comboHtml = comboText ? `<div style="font-size:12px;color:#FFD700;font-weight:900;line-height:1.6;margin-bottom:4px;">${comboText}</div>` : `<div style="font-size:12px;color:#888;line-height:1.6;margin-bottom:4px;">콤보 없음</div>`;
    if (panel) panel.innerHTML = `<div style="text-align:center;color:#fff;"><div style="font-size:32px;">✨☕✨</div>${getAlbaDifficultyBadge(cafeDifficulty)}<div style="font-size:22px;font-weight:900;color:#FFD700;margin:6px 0;">+🍔 ${cafeLastBonus}</div><div style="font-size:12px;color:#aaa;line-height:1.55;margin-bottom:4px;">기본 보상 ${baseBonus} · 총점 ${total}점</div>${comboHtml}<button onclick="initCafeAlba()" style="width:100%;padding:12px;background:linear-gradient(135deg,#FF6B9D,#C084FC);border:none;border-radius:12px;color:#fff;font-size:15px;font-weight:700;cursor:pointer;font-family:'Noto Sans KR',sans-serif;">한 잔 더! ☕</button></div>`;
  }
...
function showResult(grade, earn) {
  ...
  popup.style.borderColor = colors[grade] || '#666';
  document.getElementById('result-grade-text').style.color = colors[grade] || '#666';
  document.getElementById('result-grade-text').textContent = grade;
  document.getElementById('result-coins-text').textContent = earn > 0 ? `+🍔 ${earn}` : (messages[grade] || '💪 다음에 더 잘할 수 있어!');
