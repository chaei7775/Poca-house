...
function renderBurgerStep() {
  ...
  if (burgerStep >= menu.steps.length) {
    if (!burgerCompleted) {
      ...
      const baseByGrade = { PERFECT: 250, GREAT: 180, COOL: 180, MISS: 80 };
      const baseEarn = baseByGrade[burgerGaugeGrade] || 80;
      const cleanBonus = 20; // 현재 버거는 정해진 순서대로만 누르므로 클린 보너스 지급
      const speedBonus = elapsedSec <= 12 ? 20 : 0;
      const rawEarn = baseEarn + burgerGaugeBonus + cleanBonus + speedBonus;
      const earn = applyClothCoinBonus(applyAlbaDifficultyReward(rawEarn, burgerDifficulty, burgerGaugeGrade));
      burgerLastEarn = earn;
...
function selectBurgerMenu(idx) { stopBurgerGauge(); burgerMenuIdx = idx; burgerStep = 0; burgerStack = []; burgerGaugeLocked = false; burgerCompleted = false; burgerLastEarn = 0; burgerDifficulty = pickAlbaDifficulty(); burgerGaugeGrade = 'MISS'; burgerGaugeBonus = 0; burgerStartTime = Date.now(); renderBurgerStep(); }
...
function renderCafeStep() {
  ...
  if (cafeStep === 0) {
    ...
    const baseBonus = total >= 120 ? 450 : total >= 90 ? 320 : total >= 55 ? 220 : 100;
    ...
    if (!cafeCompleted) {
      cafeCompleted = true;
      const rawCafeReward = baseBonus + cafeComboBonus;
      cafeLastBonus = applyClothCoinBonus(applyAlbaDifficultyReward(rawCafeReward, cafeDifficulty, 'COMPLETE'));
...
const FLOWER_LEVELS = {
  easy: { label:'초급', count:4, rewards:{ FAIL:160, GOOD:300, GREAT:440, PERFECT:600 }, color:'#22c55e' },
  normal: { label:'중급', count:6, rewards:{ FAIL:240, GOOD:500, GREAT:760, PERFECT:1000 }, color:'#F59E0B' },
  hard: { label:'고급', count:8, rewards:{ FAIL:400, GOOD:800, GREAT:1300, PERFECT:1800 }, color:'#FF6B9D' }
};
...
function checkRainbowDewDrop(levelKey) {
  const rates = { easy: 0.0002, normal: 0.0005, hard: 0.001 };
...
function finishFlowerJob() {
  ...
  const earn = applyClothCoinBonus(rawEarn);
  coins += earn; albaDone++; saveAll();
