...
function showGachaResultMulti(cards) {
  ...
  const fireworks = (bestRank === 'UR' || bestRank === 'SSR') ? `<div style="position:absolute;inset:0;pointer-events:none;overflow:hidden;">
    <div style="position:absolute;left:8%;top:10%;font-size:32px;animation:fireworkPop 1.4s ease infinite;">🎆</div>
    <div style="position:absolute;right:8%;top:14%;font-size:30px;animation:fireworkPop 1.6s ease infinite .25s;">🎇</div>
    <div style="position:absolute;left:18%;bottom:14%;font-size:24px;animation:fireworkPop 1.5s ease infinite .5s;">✨</div>
    <div style="position:absolute;right:18%;bottom:18%;font-size:26px;animation:fireworkPop 1.7s ease infinite .75s;">💫</div>
  </div>` : '';
  const cardsHtml = cards.map((card, i) => {
    const g = getGachaRarityGlow(card);
    const rareBadge = (card.grade === 'UR' || card.grade === 'SSR') ? `<div style="position:absolute;top:-10px;right:-8px;background:${g.border};color:${card.grade==='UR'?'#1a1a2e':'#fff'};border:2px solid #fff;border-radius:999px;padding:4px 8px;font-size:11px;font-weight:1000;box-shadow:0 0 18px ${g.glow};">${card.grade}</div>` : `<div style="position:absolute;top:-8px;right:-6px;background:rgba(255,255,255,.92);color:${g.border};border-radius:999px;padding:3px 7px;font-size:10px;font-weight:1000;">${card.grade}</div>`;
    return `<div style="position:relative;animation:cardPop .45s ease both;animation-delay:${i*0.12}s;background:linear-gradient(180deg,rgba(255,255,255,.14),rgba(255,255,255,.04));border:2px solid ${g.border};border-radius:18px;padding:8px;width:30%;min-width:92px;max-width:118px;box-shadow:0 0 24px ${g.glow};">
      ${rareBadge}
      <div style="aspect-ratio:3/4;border-radius:12px;overflow:hidden;background:#111;margin-bottom:7px;"><img src="${card.img}" style="width:100%;height:100%;object-fit:cover;" onerror="this.style.display='none'"></div>
      <div style="font-size:10px;font-weight:1000;color:${g.border};letter-spacing:.5px;">${card.grade}</div>
...
function showGachaResult(card) {
  ...
  if (card.grade === 'UR' || card.grade === 'SSR') {
    const old = document.getElementById('gacha-single-fancy-overlay');
    if (old) old.remove();
    const overlay = document.createElement('div');
    overlay.id = 'gacha-single-fancy-overlay';
    overlay.style.cssText = `position:fixed;inset:0;z-index:320;background:radial-gradient(circle at 50% 25%,${glow.glow},rgba(10,10,30,.96) 42%,rgba(0,0,0,.96));display:flex;align-items:center;justify-content:center;padding:20px;overflow:hidden;`;
    overlay.innerHTML = `<div style="position:absolute;inset:0;pointer-events:none;">
      <div style="position:absolute;left:9%;top:12%;font-size:34px;animation:fireworkPop 1.4s ease infinite;">🎆</div>
      <div style="position:absolute;right:9%;top:15%;font-size:34px;animation:fireworkPop 1.6s ease infinite .2s;">🎇</div>
      <div style="position:absolute;left:14%;bottom:14%;font-size:28px;animation:fireworkPop 1.5s ease infinite .4s;">✨</div>
      <div style="position:absolute;right:16%;bottom:18%;font-size:28px;animation:fireworkPop 1.7s ease infinite .7s;">💫</div>
    </div>
    <div style="position:relative;z-index:2;text-align:center;width:92%;max-width:330px;border:2px solid ${glow.border};border-radius:26px;padding:18px;background:rgba(255,255,255,.08);box-shadow:0 0 42px ${glow.glow};">
      <div style="font-size:28px;font-weight:1000;color:${glow.border};font-family:'Nunito',sans-serif;letter-spacing:1.6px;text-shadow:0 0 18px ${glow.glow};">CONGRATULATIONS!</div>
      <div style="font-size:14px;color:#fff;font-weight:900;margin:3px 0 13px;">${card.grade} CARD GET!</div>
      <div style="width:230px;aspect-ratio:3/4;margin:0 auto 13px;border-radius:18px;overflow:hidden;border:3px solid ${glow.border};box-shadow:0 0 34px ${glow.glow};animation:cardReveal .55s cubic-bezier(0.34,1.56,0.64,1);background:#111;"><img src="${card.img}" style="width:100%;height:100%;object-fit:cover;" onerror="this.style.display='none'"></div>
      <div style="font-size:17px;color:#fff;font-weight:1000;margin-bottom:14px;">${card.name}</div>
      <button onclick="document.getElementById('gacha-single-fancy-overlay').remove();renderHomeIdols();renderHomeSpeech();" style="width:100%;padding:14px;background:linear-gradient(135deg,#FF6B9D,#C084FC);border:none;border-radius:15px;color:#fff;font-size:15px;font-weight:1000;font-family:'Noto Sans KR',sans-serif;">확인</button>
    </div>`;
    document.body.appendChild(overlay);
    playFanfare?.();
    return;
...
function goTo(id) {
  ...
  updateCoinsDisplay();
updateGachaCostLabels();
  if (id === 'alba') initAlba();
  if (id === 'gacha') updateGachaCostLabels();
...
function ensurePocaPatchStyles() {
  if (document.getElementById('poca-patch-styles')) return;
  const st = document.createElement('style');
  st.id = 'poca-patch-styles';
  st.textContent = `@keyframes comboPop{0%{opacity:0;transform:translate(-50%,-50%) scale(.55) rotate(-8deg)}18%{opacity:1;transform:translate(-50%,-50%) scale(1.14) rotate(3deg)}70%{opacity:1;transform:translate(-50%,-50%) scale(1)}100%{opacity:0;transform:translate(-50%,-80%) scale(.9)}} @keyframes flowerSparkle{0%{opacity:0;transform:translate(-50%,-50%) scale(.45)}25%{opacity:1;transform:translate(-50%,-50%) scale(1.15)}70%{opacity:1;transform:translate(-50%,-50%) scale(1)}100%{opacity:0;transform:translate(-50%,-50%) scale(.75)}} .flower-hotspot:active{transform:translate(-50%,-50%) scale(.92)!important;} .flower-order-icon{display:inline-flex;align-items:center;justify-content:center;width:46px;height:46px;border-radius:14px;background:#fff;color:#111;margin:3px;font-size:24px;border:2px solid #FFB3CC;box-shadow:0 2px 8px #0002;} .flower-pick-icon{display:inline-flex;align-items:center;justify-content:center;width:30px;height:30px;border-radius:10px;background:rgba(255,255,255,.92);color:#111;margin:2px;font-size:17px;} @keyframes fireworkPop{0%,100%{opacity:.25;transform:scale(.65) rotate(0deg)}45%{opacity:1;transform:scale(1.25) rotate(12deg)}70%{opacity:.75;transform:scale(.95) rotate(-8deg)}}`;
  document.head.appendChild(st);
}
ensurePocaPatchStyles();


// ── 알바 매크로 방지 / MISS 보호 ──
let albaMissStreak = parseInt(localStorage.getItem('ph_albaMissStreak') || '0');
let albaPenaltyUntil = parseInt(localStorage.getItem('ph_albaPenaltyUntil') || '0');
function getAlbaPenaltyLeftMs() { return Math.max(0, albaPenaltyUntil - Date.now()); }
function isAlbaPenaltyActive(showMsg) {
  const left = getAlbaPenaltyLeftMs();
  if (left > 0) {
    if (showMsg) showBagToast(`비정상 플레이 감지로 알바 이용 제한 중이에요. ${Math.ceil(left / 60000)}분 남음`);
    return true;
  }
  return false;
}
function recordAlbaAttempt(success, type) {
  if (success) albaMissStreak = 0;
  else albaMissStreak += 1;
  localStorage.setItem('ph_albaMissStreak', albaMissStreak);
  if (!success && albaMissStreak === 10) showMacroWarning('⚠ 비정상적인 반복 MISS가 감지됐어요. 계속되면 알바가 제한됩니다.');
  if (!success && albaMissStreak === 20) showMacroWarning('⛔ MISS가 너무 많아 알바를 강제 종료했어요. 보상은 없습니다.');
  if (!success && albaMissStreak >= 30) {
    albaPenaltyUntil = Date.now() + 30 * 60 * 1000;
    localStorage.setItem('ph_albaPenaltyUntil', albaPenaltyUntil);
    albaMissStreak = 0;
    localStorage.setItem('ph_albaMissStreak', albaMissStreak);
    showMacroWarning('🚫 자동 플레이가 의심되어 알바 이용이 30분 제한됩니다.');
  }
}
function showMacroWarning(msg) {
  const old = document.getElementById('macro-warning-overlay');
  if (old) old.remove();
  const el = document.createElement('div');
  el.id = 'macro-warning-overlay';
  el.style.cssText = 'position:fixed;inset:0;z-index:1300;background:rgba(0,0,0,.72);display:flex;align-items:center;justify-content:center;padding:20px;';
  el.innerHTML = `<div style="background:linear-gradient(135deg,#1a1a2e,#2d1b4e);border:2px solid #FF6B9D;border-radius:22px;padding:24px 20px;text-align:center;width:90%;max-width:320px;box-shadow:0 0 28px #FF6B9D66;"><div style="font-size:42px;margin-bottom:8px;">⚠️</div><div style="font-size:15px;color:#fff;font-weight:900;line-height:1.6;margin-bottom:16px;">${msg}</div><button onclick="document.getElementById('macro-warning-overlay').remove()" style="width:100%;padding:13px;background:linear-gradient(135deg,#FF6B9D,#C084FC);border:none;border-radius:14px;color:#fff;font-weight:900;font-family:'Noto Sans KR',sans-serif;">확인</button></div>`;
  document.body.appendChild(el);
}
...
let burgerGaugeGrade = 'MISS';
let burgerGaugeBonus = 0;
let burgerStartTime = 0;
...
function initBurgerAlba() {
  ...
  burgerStep = 0; burgerMenuIdx = -1; burgerStack = []; burgerGaugeLocked = false; burgerCompleted = false; burgerLastEarn = 0; burgerDifficulty = null; burgerGaugeGrade = 'MISS'; burgerGaugeBonus = 0; burgerStartTime = 0;
  renderBurgerStep();
}
function renderBurgerStep() {
  ...
  if (burgerStep >= menu.steps.length) {
    if (!burgerCompleted) {
      ...
      const elapsedSec = burgerStartTime ? Math.floor((Date.now() - burgerStartTime) / 1000) : 999;
      const baseByGrade = { PERFECT: 250, GREAT: 180, COOL: 180, MISS: 0 };
      const baseEarn = baseByGrade[burgerGaugeGrade] || 0;
      const cleanBonus = burgerGaugeGrade === 'MISS' ? 0 : 20; // MISS는 보상/보너스 없음
      const speedBonus = (burgerGaugeGrade !== 'MISS' && elapsedSec <= 12) ? 20 : 0;
    ...
    if (hotspots) hotspots.innerHTML = '';
    if (panel) panel.innerHTML = `<div style="text-align:center;color:#fff;"><div style="font-size:32px;">🍔✨</div>${getAlbaDifficultyBadge(burgerDifficulty)}<div style="font-size:20px;font-weight:900;color:#FFD700;margin:6px 0;">+🍔 ${burgerLastEarn}</div><div style="font-size:12px;color:#aaa;margin-bottom:8px;">${menu.name} 완성!</div><div style="font-size:11px;color:#FFB3CC;line-height:1.55;margin-bottom:10px;">${burgerGaugeGrade === 'MISS' ? 'MISS 보상 없음' : (burgerGaugeGrade === 'PERFECT' ? '✨ 패티 퍼펙트 +30<br>' : '') + '🔥 클린 보너스 +20' + (burgerStartTime && Math.floor((Date.now()-burgerStartTime)/1000) <= 12 ? '<br>⚡ 스피드 보너스 +20' : '')}</div><div style="display:flex;gap:8px;"><button onclick="selectBurgerMenu(${burgerMenuIdx})" style="flex:1;padding:12px;background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.2);border-radius:12px;color:#fff;font-size:14px;font-weight:700;cursor:pointer;font-family:'Noto Sans KR',sans-serif;">다시!</button><button onclick="initBurgerAlba()" style="flex:1;padding:12px;background:linear-gradient(135deg,#FF6B9D,#C084FC);border:none;border-radius:12px;color:#fff;font-size:14px;font-weight:700;cursor:pointer;font-family:'Noto Sans KR',sans-serif;">다른 메뉴</button></div></div>`;
    return;
  }
...
function selectBurgerMenu(idx) { if (isAlbaPenaltyActive(true)) return; stopBurgerGauge(); burgerMenuIdx = idx; burgerStep = 0; burgerStack = []; burgerGaugeLocked = false; burgerCompleted = false; burgerLastEarn = 0; burgerDifficulty = pickAlbaDifficulty(); burgerGaugeGrade = 'MISS'; burgerGaugeBonus = 0; burgerStartTime = Date.now(); renderBurgerStep(); }
...
function tapBurgerGauge() {
  ...
  else if (pos >= 35 && pos <= 65) { grade = 'GREAT'; burgerGaugeBonus = 0; }
  else if (pos >= 25 && pos <= 75) { grade = 'COOL'; burgerGaugeBonus = 0; }
  else { grade = 'MISS'; burgerGaugeBonus = 0; }
  burgerGaugeGrade = grade;
  showResult(grade, 0);
...
function initCafeAlba() {
  if (isAlbaPenaltyActive(true)) { goTo('alba'); return; }
  stopCafeGauge();
  if (document.getElementById('coin-alba-cafe')) document.getElementById('coin-alba-cafe').textContent = coins;
  cafeStep = 0; cafeScores = []; cafeGaugeRunning = false; cafeGaugeLocked = false; cafeCompleted = false; cafeLastBonus = 0; cafeComboBonus = 0; cafeComboText = ''; cafeDifficulty = pickAlbaDifficulty(); renderCafeStep();
}
...
function renderCafeStep() {
  ...
  if (cafeStep === 0) {
    ...
    const mainScores = cafeScores.slice(0, 3);
    const perfectCount = mainScores.filter(v => v >= 40).length;
    const baseBonus = total >= 120 ? 450 : total >= 90 ? 320 : total >= 55 ? 220 : 0;
    cafeComboBonus = (perfectCount >= 3) ? 80 : (cafeScores[0] >= 40 && cafeScores[1] >= 40) ? 30 : 0;
    cafeComboText = (perfectCount >= 3) ? '🔥 X3 COMBO +80' : (cafeComboBonus ? '🔥 X2 COMBO +30' : '');
    if (!cafeCompleted) {
      cafeCompleted = true;
...
function selectAndDrawLatte(art, emoji) {
  ...
  showResult(score >= 40 ? 'PERFECT' : 'GREAT', 0);
...
// ── 꽃집 알바 ──
const FLOWER_ITEMS = [
  {id:'rose_red', emoji:'🌹', name:'장미'}, {id:'camellia', emoji:'🌺', name:'동백'}, {id:'tulip_red', emoji:'🌷', name:'빨강튤립'}, {id:'carnation', emoji:'🌸', name:'카네이션'}, {id:'rose_pink', emoji:'🌹', name:'핑크장미'}, {id:'gerbera', emoji:'🌼', name:'거베라'},
  {id:'sunflower', emoji:'🌻', name:'해바라기'}, {id:'tulip_yellow', emoji:'🌷', name:'노랑튤립'}, {id:'freesia', emoji:'🌼', name:'프리지아'}, {id:'rose_yellow', emoji:'🌹', name:'노랑장미'}, {id:'daisy', emoji:'🌼', name:'데이지'}, {id:'marigold', emoji:'🌼', name:'금잔화'},
  {id:'lavender', emoji:'🪻', name:'라벤더'}, {id:'lisianthus', emoji:'💜', name:'리시안셔스'}, {id:'iris', emoji:'💜', name:'아이리스'}, {id:'statice', emoji:'🪻', name:'스타티스'}, {id:'anemone', emoji:'💜', name:'아네모네'}, {id:'clover', emoji:'☘️', name:'네잎클로버'},
  {id:'hydrangea', emoji:'💠', name:'수국'}, {id:'delphinium', emoji:'🩵', name:'델피늄'}, {id:'forgetme', emoji:'💙', name:'물망초'}, {id:'blue_daisy', emoji:'💙', name:'블루데이지'}, {id:'babysbreath_blue', emoji:'🩵', name:'블루안개꽃'}, {id:'nigella', emoji:'💙', name:'니겔라'},
  {id:'cherry', emoji:'🌸', name:'벚꽃'}, {id:'tulip_pink', emoji:'🌷', name:'핑크튤립'}, {id:'peony', emoji:'🌸', name:'작약'}, {id:'hydrangea_pink', emoji:'🌸', name:'핑크수국'}, {id:'babysbreath_pink', emoji:'🌸', name:'핑크안개꽃'}, {id:'cosmos', emoji:'🌸', name:'코스모스'},
  {id:'lilyvalley', emoji:'🤍', name:'은방울꽃'}, {id:'lily', emoji:'🤍', name:'백합'}, {id:'ranunculus', emoji:'🤍', name:'라넌큘러스'}, {id:'shasta', emoji:'🤍', name:'샤스타데이지'}, {id:'babysbreath', emoji:'🤍', name:'안개꽃'}, {id:'anemone_white', emoji:'🤍', name:'화이트아네모네'}
];
const FLOWER_BOUQUETS = ['봄바람 꽃다발','달빛 꽃다발','별빛 꽃다발','행복 꽃다발','우정 꽃다발','사랑 꽃다발','축하 꽃다발','감사 꽃다발','꿈결 꽃다발','무지개 꽃다발'];
const FLOWER_LEVELS = {
  easy: { label:'초급', count:4, rewards:{ FAIL:0, GOOD:300, GREAT:440, PERFECT:600 }, color:'#22c55e' },
...
const FLOWER_POS = {
  ...
  lavender:{x:10.3,y:62.8}, lisianthus:{x:24.0,y:62.9}, iris:{x:38.8,y:62.8}, statice:{x:53.7,y:62.8}, anemone:{x:68.9,y:62.8}, clover:{x:85.0,y:62.9},
  hydrangea:{x:10.4,y:73.2}, delphinium:{x:24.1,y:73.1}, forgetme:{x:39.0,y:73.1}, blue_daisy:{x:53.9,y:73.1}, babysbreath_blue:{x:69.0,y:73.1}, nigella:{x:85.1,y:73.1},
  cherry:{x:10.4,y:83.6}, tulip_pink:{x:24.1,y:83.6}, peony:{x:39.0,y:83.6}, hydrangea_pink:{x:53.9,y:83.6}, babysbreath_pink:{x:69.0,y:83.6}, cosmos:{x:85.1,y:83.6},
  lilyvalley:{x:10.4,y:94.0}, lily:{x:24.0,y:94.0}, ranunculus:{x:39.0,y:94.0}, shasta:{x:53.9,y:94.0}, babysbreath:{x:69.0,y:94.0}, anemone_white:{x:85.1,y:94.0}
};
let flowerState = null;
let flowerPreviewTimers = [];
function clearFlowerTimers(){ flowerPreviewTimers.forEach(t => clearTimeout(t)); flowerPreviewTimers = []; }
function initFlowerAlba() {
  clearFlowerTimers();
  if (document.getElementById('coin-alba-flower')) document.getElementById('coin-alba-flower').textContent = coins;
  flowerState = null;
  renderFlowerMenu();
}
function renderFlowerMenu() {
  const panel = document.getElementById('flower-bottom-panel');
  const hot = document.getElementById('flower-hotspots');
  const title = document.getElementById('flower-job-title');
  if (title) title.textContent = '🌷 포카꽃집';
  if (hot) { hot.innerHTML = ''; hot.style.pointerEvents = 'none'; }
  if (!panel) return;
  panel.innerHTML = `<div style="color:#fff;text-align:center;"><div style="font-size:18px;font-weight:1000;margin-bottom:4px;">🌷 꽃다발 주문 난이도</div><div style="font-size:12px;color:#FFB3CC;line-height:1.5;margin-bottom:12px;">순서를 5초 동안 보고, 이미지 속 꽃을 직접 눌러요.</div><div style="display:grid;grid-template-columns:1fr;gap:8px;"><button onclick="startFlowerJob('easy')" style="padding:13px;border:none;border-radius:14px;background:linear-gradient(135deg,#22c55e,#86efac);color:#fff;font-weight:1000;font-size:15px;font-family:'Noto Sans KR',sans-serif;">🌱 초급 · 꽃 4개 · 최대 600코인</button><button onclick="startFlowerJob('normal')" style="padding:13px;border:none;border-radius:14px;background:linear-gradient(135deg,#F59E0B,#FDE68A);color:#5b2a00;font-weight:1000;font-size:15px;font-family:'Noto Sans KR',sans-serif;">🌼 중급 · 꽃 6개 · 최대 1000코인</button><button onclick="startFlowerJob('hard')" style="padding:13px;border:none;border-radius:14px;background:linear-gradient(135deg,#FF6B9D,#C084FC);color:#fff;font-weight:1000;font-size:15px;font-family:'Noto Sans KR',sans-serif;">💐 고급 · 꽃 8개 · 최대 1800코인</button></div></div>`;
}
function seededFlowerRecipe(levelKey) {
  const cfg = FLOWER_LEVELS[levelKey];
  const bouquet = FLOWER_BOUQUETS[Math.floor(Math.random() * FLOWER_BOUQUETS.length)];
  const seq = [];
  for (let i=0;i<cfg.count;i++) seq.push(FLOWER_ITEMS[Math.floor(Math.random()*FLOWER_ITEMS.length)]);
  return { bouquet, seq };
}
function startFlowerJob(levelKey) {
  if (isAlbaPenaltyActive(true)) return;
  clearFlowerTimers();
  const rec = seededFlowerRecipe(levelKey);
  flowerState = { levelKey, bouquet: rec.bouquet, sequence: rec.seq, input: [], mistakes: 0, startedAt: Date.now(), phase:'preview', completed:false, combo:0, maxCombo:0 };
  renderFlowerPlay();
  playFlowerPreview();
}
function flowerOrderIcons(list, big) {
  return list.map(f => `<span class="${big ? 'flower-order-icon' : 'flower-pick-icon'}" title="${f.name}">${f.emoji}</span>`).join('');
}
function renderFlowerHotspots() {
  const hot = document.getElementById('flower-hotspots');
  if (!hot) return;
  if (!flowerState || flowerState.phase !== 'input') { hot.style.pointerEvents = 'none'; return; }
  hot.style.pointerEvents = 'auto';
  hot.innerHTML = FLOWER_ITEMS.map(f => {
    const p = FLOWER_POS[f.id];
    if (!p) return '';
    return `<button class="flower-hotspot" onclick="tapFlower('${f.id}')" aria-label="${f.name}" style="position:absolute;left:${p.x}%;top:${p.y}%;transform:translate(-50%,-50%);width:13.5%;height:7.2%;border:2px solid transparent;border-radius:18px;background:rgba(255,255,255,0.01);cursor:pointer;z-index:12;"></button>`;
  }).join('');
}
function showFlowerSparkle(flower, label) {
  const hot = document.getElementById('flower-hotspots');
  const p = FLOWER_POS[flower.id];
  if (!hot || !p) return;
  const el = document.createElement('div');
  el.style.cssText = `position:absolute;left:${p.x}%;top:${p.y}%;transform:translate(-50%,-50%);z-index:20;pointer-events:none;width:15%;height:8%;border:3px solid #FFD700;border-radius:20px;background:rgba(255,215,0,.18);box-shadow:0 0 22px #FFD700,0 0 38px #FF6B9D;display:flex;align-items:center;justify-content:center;color:#fff;font-size:16px;font-weight:1000;text-shadow:0 2px 5px #000;animation:flowerSparkle .62s ease forwards;`;
  el.innerHTML = label || '✨';
  hot.appendChild(el);
...
function renderFlowerPlay() {
  ...
  const title = document.getElementById('flower-job-title');
  const hot = document.getElementById('flower-hotspots');
  if (!flowerState || !panel) return;
  const cfg = FLOWER_LEVELS[flowerState.levelKey];
  if (title) title.textContent = `🌷 ${cfg.label} · ${flowerState.bouquet}`;
  const targetPreview = flowerState.phase === 'preview'
    ? `<div style="font-size:12px;color:#FFD700;font-weight:1000;line-height:1.5;">5초 동안 반짝이는 꽃 위치를 순서대로 기억하세요!</div>`
    : `<div style="font-size:12px;color:#FFB3CC;font-weight:1000;line-height:1.5;">이미지 속 꽃을 순서대로 터치!<br><span style="color:#FFD700;">선택 ${flowerState.input.length}/${flowerState.sequence.length} · 실수 ${flowerState.mistakes}/3</span></div>`;
  panel.innerHTML = `<div style="color:#fff;"><div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:7px;"><div style="font-size:13px;font-weight:1000;color:${cfg.color};">${cfg.label} 주문</div><div style="font-size:12px;color:#FFD700;font-weight:900;">실수 ${flowerState.mistakes}/3</div></div><div style="background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.14);border-radius:14px;padding:9px;text-align:center;margin-bottom:8px;"><div style="font-size:13px;font-weight:1000;margin-bottom:5px;">💐 ${flowerState.bouquet}</div>${targetPreview}</div><button onclick="initFlowerAlba()" style="width:100%;padding:10px;background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.2);border-radius:12px;color:#fff;font-weight:900;font-family:'Noto Sans KR',sans-serif;">주문 취소</button></div>`;
  if (hot) {
    if (flowerState.phase === 'preview') {
      hot.style.pointerEvents = 'none';
      hot.innerHTML = `<div style="position:absolute;left:50%;top:9%;transform:translateX(-50%);background:rgba(26,26,46,0.78);border:2px solid #FFD700;border-radius:18px;padding:8px 14px;color:#fff;font-size:13px;font-weight:1000;box-shadow:0 0 20px #FFD70066;z-index:30;">순서대로 반짝이는 꽃을 기억해요!</div>`;
    } else if (flowerState.phase === 'input') {
      hot.innerHTML = '';
      renderFlowerHotspots();
    } else {
      hot.innerHTML = '';
      hot.style.pointerEvents = 'none';
    }
...
function playFlowerPreview() {
  if (!flowerState) return;
  flowerState.phase = 'preview';
  renderFlowerPlay();
  flowerState.sequence.forEach((flower, idx) => {
    const t = setTimeout(() => showFlowerSparkle(flower, `${idx+1}`), 450 + idx * 520);
    flowerPreviewTimers.push(t);
  });
  flowerPreviewTimers.push(setTimeout(() => {
    if (!flowerState) return;
    flowerState.phase = 'input';
    flowerState.startedAt = Date.now();
    renderFlowerPlay();
    showBagToast('🌷 이제 이미지 속 꽃을 순서대로 눌러요!');
  }, 5000));
}
function tapFlower(flowerId) {
  if (!flowerState || flowerState.phase !== 'input' || flowerState.completed) return;
  const flower = FLOWER_ITEMS.find(f => f.id === flowerId);
  const idx = flowerState.input.length;
  const target = flowerState.sequence[idx];
  if (!flower || !target) return;
  if (flower.id !== target.id) {
    flowerState.mistakes++;
    flowerState.combo = 0;
    showComboPopup('앗! 다른 꽃이야');
    showFlowerSparkle(flower, '×');
    if (flowerState.mistakes >= 3) { flowerState.input.push(flower); return finishFlowerJob(); }
  } else {
    flowerState.combo++;
    flowerState.maxCombo = Math.max(flowerState.maxCombo || 0, flowerState.combo);
    showFlowerTinyPop(flower.emoji);
    showFlowerSparkle(flower, '✓');
    if (flowerState.combo >= 2) showComboPopup(`🔥 X${flowerState.combo} COMBO!`);
  }
  flowerState.input.push(flower);
  if (flowerState.input.length >= flowerState.sequence.length) finishFlowerJob();
  else renderFlowerPlay();
}
function showFlowerTinyPop(text) {
  const el = document.createElement('div');
  el.style.cssText = 'position:fixed;left:50%;top:42%;transform:translate(-50%,-50%);z-index:970;background:rgba(255,255,255,0.95);border:2px solid #FF6B9D;border-radius:50%;width:54px;height:54px;display:flex;align-items:center;justify-content:center;font-size:28px;box-shadow:0 0 18px #FF6B9D55;pointer-events:none;animation:floatUp 0.8s ease forwards;';
  el.textContent = text;
  document.body.appendChild(el);
  setTimeout(() => el.remove(), 850);
}
function checkRainbowDewDrop(levelKey) {
  const rates = { easy: 0.0005, normal: 0.0005, hard: 0.0005 };
  const rate = rates[levelKey] || 0.0002;
  if (Math.random() >= rate) return false;
  addToBag('🌈', '무지개색 이슬', 'alchemy', 1, '새벽에 꽃잎 위에 맺히는 전설의 이슬. 연금술 재료로 사용된다.');
  const popup = document.createElement('div');
  popup.style.cssText = 'position:fixed;inset:0;z-index:980;background:rgba(0,0,0,0.78);display:flex;align-items:center;justify-content:center;';
  popup.innerHTML = `<div style="background:linear-gradient(135deg,#1a1a2e,#2d1b4e);border:2px solid #FFD700;border-radius:24px;padding:28px 22px;text-align:center;width:88%;max-width:300px;box-shadow:0 0 35px #C084FC77;"><div style="font-size:58px;margin-bottom:8px;filter:drop-shadow(0 0 18px #FFD700);">🌈</div><div style="font-size:21px;font-weight:1000;color:#FFD700;margin-bottom:6px;">무지개색 이슬 발견!</div><div style="font-size:13px;color:#FFB3CC;line-height:1.5;margin-bottom:16px;">꽃집에서만 아주 드물게 발견되는<br>연금술 재료예요.</div><button onclick="this.closest('div[style*=fixed]').remove()" style="padding:11px 28px;background:linear-gradient(135deg,#FF6B9D,#C084FC);border:none;border-radius:14px;color:#fff;font-weight:1000;font-family:'Noto Sans KR',sans-serif;">확인</button></div>`;
  document.body.appendChild(popup);
  setTimeout(() => { if (popup.parentNode) popup.remove(); }, 5000);
  return true;
}
function finishFlowerJob() {
  if (!flowerState || flowerState.completed) return;
  flowerState.completed = true;
  flowerState.phase = 'result';
  const cfg = FLOWER_LEVELS[flowerState.levelKey];
  const elapsed = Math.floor((Date.now() - flowerState.startedAt) / 1000);
  let grade = 'FAIL';
  if (flowerState.mistakes === 0) grade = 'PERFECT';
  else if (flowerState.mistakes === 1) grade = 'GREAT';
  else if (flowerState.mistakes === 2) grade = 'GOOD';
  if (grade === 'PERFECT') showComboPopup('✨ PERFECT!');
  const rawEarn = cfg.rewards[grade] || cfg.rewards.FAIL;
  ...
  recordAlbaAttempt(earn > 0, 'flower');
  if (document.getElementById('coin-alba-flower')) document.getElementById('coin-alba-flower').textContent = coins;
  spawnCoinFloat(earn);
  const panel = document.getElementById('flower-bottom-panel');
  const hot = document.getElementById('flower-hotspots');
  if (hot) { hot.innerHTML = ''; hot.style.pointerEvents = 'none'; }
  const seq = flowerState.sequence.map(f => `${f.emoji}${f.name}`).join(' → ');
  const comboLine = (flowerState.maxCombo || 0) >= 2 ? `<br>최고 콤보 X${flowerState.maxCombo}` : '';
  if (panel) panel.innerHTML = `<div style="text-align:center;color:#fff;"><div style="font-size:44px;margin-bottom:4px;">💐</div><div style="font-size:26px;font-weight:1000;color:${grade==='PERFECT'?'#FFD700':grade==='GREAT'?'#FF6B9D':grade==='GOOD'?'#C084FC':'#aaa'};">${grade}</div><div style="font-size:22px;font-weight:1000;color:#FFD700;margin:8px 0;">+🍔 ${earn}</div><div style="font-size:12px;color:#FFB3CC;line-height:1.5;margin-bottom:6px;">${cfg.label} · ${flowerState.bouquet}<br>실수 ${flowerState.mistakes}회 · ${elapsed}초${comboLine}</div><div style="font-size:11px;color:#ddd;background:rgba(255,255,255,0.08);border-radius:12px;padding:8px;margin-bottom:10px;line-height:1.5;">정답 순서<br>${seq}</div><div style="display:flex;gap:8px;"><button onclick="startFlowerJob('${flowerState.levelKey}')" style="flex:1;padding:12px;background:rgba(255,255,255,0.12);border:1px solid rgba(255,255,255,0.2);border-radius:12px;color:#fff;font-weight:900;font-family:'Noto Sans KR',sans-serif;">같은 난이도</button><button onclick="renderFlowerMenu()" style="flex:1;padding:12px;background:linear-gradient(135deg,#FF6B9D,#C084FC);border:none;border-radius:12px;color:#fff;font-weight:900;font-family:'Noto Sans KR',sans-serif;">난이도 선택</button></div></div>`;
}

// ── 공통 유틸 ──
function updateCoinsDisplay() {
  document.querySelectorAll('#coin-display,#coin-alba,#coin-gacha,#coin-gacha2,#coin-bag,#coin-alba-flower').forEach(el => { if (el) el.textContent = coins; });
}
function saveAll() {
...
function showResult(grade, earn) {
  ...
  if (!popup || !bg) return;
  popup.style.borderColor = colors[grade] || '#999';
  document.getElementById('result-grade-text').style.color = colors[grade] || '#999';
  document.getElementById('result-grade-text').textContent = grade;
  document.getElementById('result-coins-text').textContent = earn > 0 ? `+🍔 ${earn}` : (messages[grade] || messages.MISS);
...
function showComboPopup(text) {
  ...
  const el = document.createElement('div');
  el.id = 'combo-popup';
  el.style.cssText = 'position:fixed;left:50%;top:38%;transform:translate(-50%,-50%) scale(0.6);z-index:980;background:linear-gradient(135deg,#FFD700,#FF6B9D,#C084FC);color:#fff;border:3px solid #fff;border-radius:22px;padding:13px 24px;font-size:24px;font-weight:1000;text-shadow:0 2px 8px #0006;box-shadow:0 0 30px #FFD70088;pointer-events:none;animation:comboPop 0.9s ease forwards;';
  el.textContent = text;
  document.body.appendChild(el);
...
const PLACE_BUTTONS = {
  ...
  lake: ['btn-meet-ara', 'btn-explore-lake'],
  room: ['btn-room-deco'],
...
function showLevelUpPopup() {
  const old = document.getElementById('levelup-toast');
  if (old) old.remove();
  const el = document.createElement('div');
  el.id = 'levelup-toast';
  ...
  el.innerHTML = `⭐ 레벨 업! Lv.${playerLevel}${playerLevel === 10 ? '<br><span style="color:#FFB3CC;">🎓 기술학원 입학 가능!</span>' : ''}`;
  document.body.appendChild(el);
...
function useCoupon() {
  ...
  if (usedCoupons.includes(coupon)) { alert('이미 사용한 쿠폰입니다!'); return; }
  if (coupon === 'BETA2026') {
    normalizeSchoolDaily?.();
    coins += 30000;
    if (typeof schoolDaily !== 'undefined') schoolDaily.tickets = (schoolDaily.tickets || 0) + 3;
  ...
  if (coupon === 'TESTERCRAFT') {
    ...
    craftMaterials.forEach(([emoji, name]) => {
      addToBag(emoji, name, 'material', 10, '재봉 제작 재료');
    });
    usedCoupons.push(coupon);
    localStorage.setItem('ph_usedCoupons', JSON.stringify(usedCoupons));
    saveBag();
...
function startExplore(placeId) {
  ...
  for (let i = 0; i < itemCount; i++) {
    ...
    // 소원조각 드랍 체크
    const wishChance = (placeId === 'mystery') ? 0.01 : 0.005;
...
function normalizeClothBagItems() {
  let changed = false;
  if (typeof CLOTH_ITEMS === 'undefined') return;
  bagItems.forEach(item => {
    if (item.type !== 'cloth') return;
    let cloth = CLOTH_ITEMS.find(c => c.id === item.clothId)
...
function findClothBagItem(clothId, itemName) {
  ...
  const clean = (itemName || '').replace(/^✨\s*/, '').trim();
  if (itemName) {
    const exact = bagItems.find(i => i.type === 'cloth' && (i.name === itemName || i.name.replace(/^✨\s*/, '').trim() === clean));
    if (exact) return exact;
  }
  return bagItems.find(i => i.type === 'cloth' && (
    i.clothId === clothId ||
    i.name === cloth.name ||
    i.name === `✨ ${cloth.name}` ||
    i.name.replace(/^✨\s*/, '').trim() === cloth.name
  )) || null;
}
...
function equipCloth(clothId, itemName) {
  normalizeClothBagItems?.();
  const cloth = CLOTH_ITEMS.find(c => c.id === clothId);
  if (!cloth) { showBagToast('의상 정보를 찾지 못했어요!'); return; }
  const item = findClothBagItem(clothId, itemName);
  if (!item) { showBagToast('보유한 의상이 아니에요!'); return; }
  equippedCloth = { id: cloth.id, name: item.name, statVal: item.statVal || cloth.statVal, isGreat: !!item.isGreat };
  ...
  const detail = document.getElementById('cloth-detail-overlay');
...
// ── 초기화 ──
try { normalizeClothBagItems?.(); } catch(e) {}
...
// ════════════════════════════════
// 🏫 학교 미니게임 v1 - 받아쓰기 / 암기카드 / 칸밟기
// ════════════════════════════════
let honorStars = parseInt(localStorage.getItem('ph_honorStars') || '0');
let honorFragments = parseInt(localStorage.getItem('ph_honorFragments') || '0');
let schoolDaily = JSON.parse(localStorage.getItem('ph_schoolDaily') || '{}');
let schoolScores = JSON.parse(localStorage.getItem('ph_schoolScores') || '{}');
let schoolState = null;
let schoolSelectedCardId = localStorage.getItem('ph_schoolSelectedCardId') || '';
...
function normalizeSchoolDaily() {
  ...
  // v2.1: 수업 완료/성적표 상태는 포카별로 분리한다.
  // 예전 저장 데이터(done/report)는 현재 선택 포카에만 귀속시켜서 다른 포카 교체 시 수업이 열리게 보정.
  if (schoolDaily.done && Object.keys(schoolDaily.done).length && !schoolDaily._migratedPerCard) {
    const key = schoolSelectedCardId || '_default';
    schoolDaily.cards[key] = { done: { ...schoolDaily.done }, report: !!schoolDaily.report };
    schoolDaily._migratedPerCard = true;
  }
  if (!schoolDaily.done) schoolDaily.done = {};
  if (typeof schoolDaily.report !== 'boolean') schoolDaily.report = false;
}
normalizeSchoolDaily();

function getSchoolCardDailyKey() {
  const card = getSchoolSelectedCard?.();
  return card ? card.id : (schoolSelectedCardId || '_default');
}

function getSchoolCardDaily() {
  normalizeSchoolDaily();
  const key = getSchoolCardDailyKey();
  if (!schoolDaily.cards) schoolDaily.cards = {};
  if (!schoolDaily.cards[key]) schoolDaily.cards[key] = { done: {}, report: false };
  if (!schoolDaily.cards[key].done) schoolDaily.cards[key].done = {};
  if (typeof schoolDaily.cards[key].report !== 'boolean') schoolDaily.cards[key].report = false;
  return schoolDaily.cards[key];
}

function saveSchoolDaily() {
  normalizeSchoolDaily();
  localStorage.setItem('ph_schoolDaily', JSON.stringify(schoolDaily));
}

function addHonorFragments(amount) {
  honorFragments += amount;
  let converted = 0;
  while (honorFragments >= 100) {
    honorFragments -= 100;
    honorStars += 1;
    converted += 1;
  }
...
function canStartSchoolSubject(subject) {
  normalizeSchoolDaily();
  const cardDaily = getSchoolCardDaily();
  if (cardDaily.done[subject]) {
    showBagToast('이 포카는 오늘 이미 완료한 수업이에요! 다른 포카는 수업할 수 있어요 🎒');
    return false;
  }
  if (schoolDaily.tickets <= 0) {
    showBagToast('등교티켓이 부족해요! 🎫 내일 다시 충전돼요');
    return false;
  }
  return true;
}

function completeSchoolSubject(subject, score) {
  normalizeSchoolDaily();
  const cardDaily = getSchoolCardDaily();
  if (cardDaily.done[subject]) return false;
  schoolDaily.tickets = Math.max(0, schoolDaily.tickets - 1);
  cardDaily.done[subject] = true;
  saveSchoolDaily();
  setSchoolScore(subject, score);
  return true;
}

function migrateSchoolScores() {
  // 예전 전역 점수 저장값을 포카별 점수 저장 방식으로 보정
  if (schoolScores && (typeof schoolScores.korean === 'number' || typeof schoolScores.memory === 'number' || typeof schoolScores.pe === 'number')) {
    const old = { korean: schoolScores.korean, memory: schoolScores.memory, pe: schoolScores.pe };
    schoolScores = { _default: old };
    localStorage.setItem('ph_schoolScores', JSON.stringify(schoolScores));
  }
}
migrateSchoolScores();

function saveSchool() {
  localStorage.setItem('ph_honorStars', honorStars);
  localStorage.setItem('ph_honorFragments', honorFragments);
  localStorage.setItem('ph_schoolDaily', JSON.stringify(schoolDaily));
  localStorage.setItem('ph_schoolScores', JSON.stringify(schoolScores));
  localStorage.setItem('ph_schoolSelectedCardId', schoolSelectedCardId || '');
}

function getSchoolOwnedCards() {
  const ids = Array.from(new Set(owned || []));
  return ids.map(id => CARDS.find(c => c.id === id)).filter(Boolean);
}

function getSchoolSelectedCard() {
  const cards = getSchoolOwnedCards();
  if (!cards.length) return null;
  let card = cards.find(c => c.id === schoolSelectedCardId);
  if (!card) {
    card = cards[0];
    schoolSelectedCardId = card.id;
    saveSchool();
  }
  return card;
}
...
function getSchoolScores() {
  normalizeSchoolDaily();
  const card = getSchoolSelectedCard();
  const key = card ? card.id : '_default';
  if (!schoolScores[key] || schoolScores[key]._date !== schoolDaily.date) schoolScores[key] = { _date: schoolDaily.date };
  return schoolScores[key];
}

function setSchoolScore(subject, score) {
  const card = getSchoolSelectedCard();
  const key = card ? card.id : '_default';
  if (!schoolScores[key]) schoolScores[key] = {};
  schoolScores[key][subject] = score;
  saveSchool();
}

function clearSchoolScoresForSelected() {
  const card = getSchoolSelectedCard();
  const key = card ? card.id : '_default';
  schoolScores[key] = {};
  const cardDaily = getSchoolCardDaily();
  cardDaily.done = {};
  cardDaily.report = false;
  saveSchool();
}

function ensureSchoolButton() {
  const panel = document.querySelector('#place-overlay > div:last-child');
  if (!panel || document.getElementById('btn-school-games')) return;
  const btn = document.createElement('button');
  btn.id = 'btn-school-games';
  btn.textContent = '🎒 등교시키기';
  btn.onclick = openSchoolGames;
  btn.style.cssText = "display:none;width:100%;padding:14px;background:rgba(96,165,250,0.22);border:1.5px solid #60a5fa;border-radius:12px;color:#fff;font-size:15px;font-weight:900;cursor:pointer;font-family:'Noto Sans KR',sans-serif;";
  panel.insertBefore(btn, panel.firstChild);
}

function ensureSchoolOverlay() {
  let ov = document.getElementById('school-overlay');
  if (ov) return ov;
  ov = document.createElement('div');
  ov.id = 'school-overlay';
  ov.style.cssText = 'display:none;position:fixed;inset:0;z-index:900;background:linear-gradient(180deg,#eff6ff,#fff0f5);overflow-y:auto;max-width:430px;margin:0 auto;left:50%;transform:translateX(-50%);box-shadow:0 0 0 9999px rgba(0,0,0,0.45);font-family:\'Noto Sans KR\',sans-serif;';
...
function openSchoolGames() {
  closePlace();
  renderSchoolHome();
}
function closeSchoolGames() {
  const ov = document.getElementById('school-overlay');
...
function renderSchoolHome() {
  ...
  const card = getSchoolSelectedCard();
  if (!card) {
    ov.innerHTML = schoolHeader('연성고등학교') + `
      <div style="padding:28px 16px 120px;text-align:center;">
        <div style="background:#fff;border:2px solid #bfdbfe;border-radius:24px;padding:26px 18px;box-shadow:0 6px 22px #60a5fa22;">
          <div style="font-size:46px;margin-bottom:8px;">🎒</div>
          <div style="font-size:20px;font-weight:900;color:#1d4ed8;margin-bottom:8px;">등교시킬 포카가 없어요</div>
          <div style="font-size:13px;color:#64748b;line-height:1.6;margin-bottom:18px;">카드 뽑기에서 포카를 먼저 뽑은 뒤 연성고등학교에 보내주세요.</div>
          <button onclick="closeSchoolGames();goTo('gacha')" style="width:100%;padding:14px;border:none;border-radius:14px;background:linear-gradient(135deg,#C084FC,#FF6B9D);color:#fff;font-size:15px;font-weight:900;font-family:'Noto Sans KR',sans-serif;cursor:pointer;">✨ 카드 뽑으러 가기</button>
        </div>
      </div>`;
  ...
  const cardDaily = getSchoolCardDaily();
  const k = scores.korean;
  const m = scores.memory;
  const p = scores.pe;
  const allDone = [k,m,p].every(v => typeof v === 'number');
  const canReport = allDone && !cardDaily.report;
  ov.innerHTML = schoolHeader('연성고등학교') + `
    <div style="padding:10px 12px 110px;display:flex;flex-direction:column;gap:9px;">
      <div style="background:#fff;border:2px solid #bfdbfe;border-radius:18px;padding:10px 11px;box-shadow:0 4px 14px #60a5fa22;display:flex;align-items:center;gap:9px;min-width:0;">
        <img class="school-card-img" src="${card.img}" style="width:38px;height:50px;object-fit:cover;border-radius:8px;background:#eee;flex-shrink:0;" onerror="this.style.display='none'">
        <div style="flex:1;min-width:0;">
          <div style="font-size:14px;font-weight:900;color:#1d4ed8;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">🎒 ${card.name} 등교 중</div>
          <div style="font-size:10.5px;color:#64748b;line-height:1.35;margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">수업 1회 🎫1장 · 성적표 보상</div>
        </div>
        <button onclick="renderSchoolSelect()" style="border:none;border-radius:11px;background:#eff6ff;color:#1d4ed8;font-weight:900;padding:8px 9px;cursor:pointer;font-family:'Noto Sans KR',sans-serif;font-size:12px;white-space:nowrap;flex-shrink:0;">바꾸기</button>
      </div>
      ${schoolSubjectCard('📚','국어 · 받아쓰기','보기 중 맞는 단어 고르기',k,'startDictationGame()')}
      ${schoolSubjectCard('🧠','수학 · 암기카드','9장 카드 위치 기억하기',m,'startMemoryCardGame()')}
      ${schoolSubjectCard('🏃','체육 · 칸밟기','12칸 중 초록칸 6개 기억하기',p,'startStepTileGame()')}
      <button onclick="finishSchoolReport()" ${canReport ? '' : 'disabled'} style="width:100%;padding:16px;border:none;border-radius:16px;font-size:16px;font-weight:900;font-family:'Noto Sans KR',sans-serif;cursor:${canReport ? 'pointer' : 'not-allowed'};background:${canReport ? 'linear-gradient(135deg,#F59E0B,#FF6B9D)' : '#ddd'};color:${canReport ? '#fff' : '#999'};box-shadow:${canReport ? '0 4px 18px #F59E0B44' : 'none'};">${cardDaily.report ? '✅ 이 포카 성적표 완료' : '📋 성적표 받기'}</button>
    </div>`;
  ov.style.display = 'block';
}
...
function renderSchoolSelect() {
  const ov = ensureSchoolOverlay();
  const cards = getSchoolOwnedCards();
  ov.innerHTML = schoolHeader('🎒 등교시킬 포카 선택') + `
    <div style="padding:18px 16px 120px;display:flex;flex-direction:column;gap:10px;">
      ${cards.map(c => `<button onclick="selectSchoolCard('${c.id}')" style="width:100%;padding:12px;background:#fff;border:2px solid ${c.id === schoolSelectedCardId ? '#60a5fa' : '#fbcfe8'};border-radius:18px;display:flex;align-items:center;gap:12px;text-align:left;font-family:'Noto Sans KR',sans-serif;cursor:pointer;box-shadow:0 3px 12px #00000010;">
        <img class="school-card-img" src="${c.img}" style="width:44px;height:58px;object-fit:cover;border-radius:8px;background:#eee;flex-shrink:0;" onerror="this.style.display='none'">
        <div style="flex:1;"><div style="font-size:15px;font-weight:900;color:#111;">${c.name}</div><div style="font-size:12px;color:#777;margin-top:2px;">${c.grade} · 연성고등학교 등교 가능</div></div>
        <div style="font-size:20px;">${c.id === schoolSelectedCardId ? '✅' : '🎒'}</div>
      </button>`).join('')}
    </div>`;
  ov.style.display = 'block';
}

function selectSchoolCard(cardId) {
  schoolSelectedCardId = cardId;
  saveSchool();
  renderSchoolHome();
}

function schoolSubjectCard(icon, title, desc, score, action) {
  normalizeSchoolDaily();
  const cardDaily = getSchoolCardDaily();
  const subject = action.includes('Dictation') ? 'korean' : action.includes('Memory') ? 'memory' : 'pe';
  const doneToday = !!cardDaily.done[subject];
  const scoreText = typeof score === 'number' ? `${score}점` : doneToday ? '완료' : '미완료';
  const disabled = doneToday || schoolDaily.tickets <= 0;
  return `<button onclick="${disabled ? '' : action}" ${disabled ? 'disabled' : ''} style="text-align:left;width:100%;padding:11px 12px;background:${doneToday ? '#f8fafc' : '#fff'};border:2px solid ${doneToday ? '#cbd5e1' : '#fbcfe8'};border-radius:16px;box-shadow:0 3px 12px #FF6B9D14;display:flex;align-items:center;gap:10px;font-family:'Noto Sans KR',sans-serif;cursor:${disabled ? 'not-allowed' : 'pointer'};opacity:${disabled && !doneToday ? '0.62' : '1'};">
    <div style="font-size:28px;width:38px;text-align:center;flex-shrink:0;">${icon}</div>
    <div style="flex:1;min-width:0;"><div style="font-size:14px;font-weight:900;color:#111;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${title}</div><div style="font-size:10.5px;color:#777;margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${desc} · 🎫1</div></div>
    <div style="font-size:12px;font-weight:900;color:${typeof score === 'number' ? '#F59E0B' : doneToday ? '#64748b' : '#94a3b8'};white-space:nowrap;flex-shrink:0;">${scoreText}</div>
  </button>`;
}

function resetSchoolScores() {
  clearSchoolScoresForSelected();
  renderSchoolHome();
  showBagToast('🏫 학교 점수를 다시 시작했어요!');
}

// 📚 받아쓰기
const DICTATION_QUESTIONS = [
  { word:'벚꽃', options:['벚꽃','벗꽃','벛꽃','벚꼿'] },
  { word:'무지개', options:['무지개','무지계','무지게','무지걔'] },
  { word:'은방울꽃', options:['은방울꽃','은방울꼿','은바울꽃','은방울꽂'] },
  { word:'포카하우스', options:['포카하우스','포카하스','포카하우쓰','포카하우즈'] },
  { word:'우등생별', options:['우등생별','우등생볓','우등샘별','우등생뼐'] },
  { word:'신비의 섬', options:['신비의 섬','신비에 섬','신비의 썸','신뷔의 섬'] },
  { word:'무지개수정', options:['무지개수정','무지계수정','무지개수젱','무지개수졍'] }
];
function startDictationGame() {
  if (!canStartSchoolSubject('korean')) { renderSchoolHome(); return; }
  const qs = [...DICTATION_QUESTIONS].sort(() => Math.random() - 0.5).slice(0,5);
  schoolState = { type:'korean', qs, idx:0, correct:0 };
...
function renderDictationQuestion() {
  ...
  ov.innerHTML = schoolHeader('📚 국어 · 받아쓰기') + `
    <div style="padding:20px 16px 120px;">
      <div style="background:#fff;border:2px solid #bfdbfe;border-radius:22px;padding:22px 16px;text-align:center;box-shadow:0 5px 20px #60a5fa22;">
        <div style="font-size:12px;font-weight:900;color:#60a5fa;margin-bottom:10px;">문제 ${st.idx+1} / ${st.qs.length}</div>
        <div style="font-size:13px;color:#777;margin-bottom:8px;">선생님이 읽어준 단어를 고르세요</div>
        <div style="font-size:28px;font-weight:900;color:#1d4ed8;background:#eff6ff;border-radius:18px;padding:18px;margin-bottom:16px;">“${q.word}”</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">${opts.map(o => `<button onclick="answerDictation('${o.replace(/'/g,"\\'")}')" style="padding:15px 8px;border:none;border-radius:14px;background:linear-gradient(135deg,#FF6B9D,#C084FC);color:#fff;font-size:16px;font-weight:900;font-family:'Noto Sans KR',sans-serif;cursor:pointer;">${o}</button>`).join('')}</div>
      </div>
    </div>`;
  ov.style.display = 'block';
}
function answerDictation(ans) {
  const st = schoolState;
  if (!st || st.type !== 'korean') return;
  if (ans === st.qs[st.idx].word) st.correct++;
  st.idx++;
  if (st.idx >= st.qs.length) {
    const score = Math.round((st.correct / st.qs.length) * 100);
    completeSchoolSubject('korean', score);
    showSchoolResult('📚 받아쓰기 완료!', score, `${st.correct}/${st.qs.length} 정답`);
  } else renderDictationQuestion();
}

// 🧠 암기카드 9장
function startMemoryCardGame() {
  if (!canStartSchoolSubject('memory')) { renderSchoolHome(); return; }
  const icons = ['🍓','⭐','🌸','🍔'];
  const cards = [...icons, ...icons, '💎'].sort(() => Math.random() - 0.5).map((v,i) => ({ id:i, v, open:true, done:false }));
  schoolState = { type:'memory', cards, first:null, pairs:0, bonus:false, moves:0, locked:true };
  renderMemoryCards('3초 동안 위치를 기억하세요!');
  setTimeout(() => {
    if (!schoolState || schoolState.type !== 'memory') return;
    schoolState.cards.forEach(c => { c.open = false; });
    schoolState.locked = false;
    renderMemoryCards('같은 그림 4쌍을 찾아요. 💎는 보너스!');
  }, 3000);
}
function renderMemoryCards(msg) {
  const ov = ensureSchoolOverlay();
  const st = schoolState;
  ov.innerHTML = schoolHeader('🧠 수학 · 암기카드') + `
    <div style="padding:20px 16px 120px;text-align:center;">
      <div style="background:#fff;border:2px solid #ddd6fe;border-radius:22px;padding:18px;box-shadow:0 5px 20px #C084FC22;">
        <div style="font-size:13px;font-weight:900;color:#7c3aed;margin-bottom:12px;">${msg}</div>
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;max-width:270px;margin:0 auto;">
          ${st.cards.map((c,i) => `<button onclick="flipMemoryCard(${i})" style="aspect-ratio:1/1;border:none;border-radius:16px;background:${c.done ? '#dcfce7' : c.open ? '#fff7ed' : 'linear-gradient(135deg,#60a5fa,#C084FC)'};font-size:34px;font-weight:900;box-shadow:0 3px 10px #0001;cursor:pointer;">${(c.open || c.done) ? c.v : '?'}</button>`).join('')}
        </div>
        <div style="font-size:12px;color:#777;margin-top:14px;">찾은 쌍 ${st.pairs}/4 · 터치 ${st.moves}회</div>
      </div>
    </div>`;
  ov.style.display = 'block';
}
function flipMemoryCard(i) {
  const st = schoolState;
  if (!st || st.type !== 'memory' || st.locked) return;
  const c = st.cards[i];
  if (!c || c.done || c.open) return;
  c.open = true; st.moves++;
  if (c.v === '💎') { c.done = true; st.bonus = true; renderMemoryCards('💎 보너스 발견! 계속 찾아요.'); return; }
  if (st.first === null) { st.first = i; renderMemoryCards('한 장 더 골라요!'); return; }
  const f = st.cards[st.first];
  if (f.v === c.v) {
    f.done = true; c.done = true; st.pairs++; st.first = null;
    if (st.pairs >= 4) {
      const penalty = Math.max(0, st.moves - 8) * 3;
      const score = Math.max(50, Math.min(100, 100 - penalty + (st.bonus ? 5 : 0)));
      completeSchoolSubject('memory', score);
      showSchoolResult('🧠 암기카드 완료!', score, `${st.moves}번 터치`);
    } else renderMemoryCards('정답! 다음 쌍을 찾아요.');
  } else {
    st.locked = true; renderMemoryCards('앗 달라요!');
    setTimeout(() => { if (!schoolState || schoolState.type !== 'memory') return; f.open = false; c.open = false; st.first = null; st.locked = false; renderMemoryCards('다시 기억해서 찾아요!'); }, 650);
  }
}

// 🏃 칸밟기 12칸 / 6개
function startStepTileGame() {
  if (!canStartSchoolSubject('pe')) { renderSchoolHome(); return; }
  const greens = [];
  while (greens.length < 6) { const n = Math.floor(Math.random()*12); if (!greens.includes(n)) greens.push(n); }
  schoolState = { type:'pe', greens, selected:[], wrong:0, show:true };
  renderStepTiles('초록칸 6개를 기억하세요!');
  setTimeout(() => { if (!schoolState || schoolState.type !== 'pe') return; schoolState.show = false; renderStepTiles('기억한 초록칸을 눌러요!'); }, 2600);
}
function renderStepTiles(msg) {
  const ov = ensureSchoolOverlay();
  const st = schoolState;
  ov.innerHTML = schoolHeader('🏃 체육 · 칸밟기') + `
    <div style="padding:20px 16px 120px;text-align:center;">
      <div style="background:#fff;border:2px solid #bbf7d0;border-radius:22px;padding:18px;box-shadow:0 5px 20px #22c55e22;">
        <div style="font-size:13px;font-weight:900;color:#16a34a;margin-bottom:12px;">${msg}</div>
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:9px;max-width:300px;margin:0 auto;">
          ${Array.from({length:12}).map((_,i) => {
            const isGreen = st.greens.includes(i);
            const sel = st.selected.includes(i);
            const bg = st.show && isGreen ? '#22c55e' : sel ? (isGreen ? '#86efac' : '#fecaca') : '#e5e7eb';
            const txt = st.show && isGreen ? '🟩' : sel ? (isGreen ? '✓' : '✕') : '';
            return `<button onclick="tapStepTile(${i})" style="aspect-ratio:1/1;border:none;border-radius:14px;background:${bg};font-size:22px;font-weight:900;color:#111;box-shadow:0 3px 9px #0001;cursor:pointer;">${txt}</button>`;
          }).join('')}
        </div>
        <div style="font-size:12px;color:#777;margin-top:14px;">선택 ${st.selected.length}/6 · 실수 ${st.wrong}</div>
      </div>
    </div>`;
  ov.style.display = 'block';
}
function tapStepTile(i) {
  const st = schoolState;
  if (!st || st.type !== 'pe' || st.show || st.selected.includes(i)) return;
  st.selected.push(i);
  if (!st.greens.includes(i)) st.wrong++;
  if (st.selected.length >= 6) {
    const correct = st.selected.filter(x => st.greens.includes(x)).length;
    const score = Math.max(0, Math.min(100, Math.round(correct * 16.7 - st.wrong * 8)));
    completeSchoolSubject('pe', score);
    showSchoolResult('🏃 칸밟기 완료!', score, `정답 ${correct}/6`);
  } else renderStepTiles('계속 눌러요!');
}

function showSchoolResult(title, score, sub) {
  const ov = ensureSchoolOverlay();
  ov.innerHTML = schoolHeader(title) + `
    <div style="padding:34px 16px 120px;text-align:center;">
      <div style="background:#fff;border:2px solid #fbcfe8;border-radius:24px;padding:28px 18px;box-shadow:0 6px 22px #FF6B9D22;">
        <div style="font-size:52px;font-weight:900;color:#F59E0B;margin-bottom:4px;">${score}</div>
        <div style="font-size:14px;color:#777;margin-bottom:20px;">${sub}</div>
        <button onclick="renderSchoolHome()" style="width:100%;padding:14px;border:none;border-radius:14px;background:linear-gradient(135deg,#60a5fa,#C084FC);color:#fff;font-size:15px;font-weight:900;font-family:'Noto Sans KR',sans-serif;cursor:pointer;">학교로 돌아가기</button>
      </div>
    </div>`;
}
...
function finishSchoolReport() {
  normalizeSchoolDaily();
  const cardDaily = getSchoolCardDaily();
  if (cardDaily.report) { showBagToast('이 포카 성적표는 이미 받았어요!'); return; }
  const scores = getSchoolScores();
  ...
  else if (avg >= 70) { rank = 'B'; fragmentReward = 7; }
  honorStars += starReward;
  const converted = addHonorFragments(fragmentReward);
  cardDaily.report = true;
  saveSchoolDaily();
  addPlayerExp(8);
  const ov = ensureSchoolOverlay();
  const rewardText = `${starReward ? `우등생별 +${starReward} ⭐<br>` : ''}우등생조각 +${fragmentReward} ✨${converted ? `<br><span style="color:#F59E0B;">조각 100개 완성! 우등생별 +${converted} ⭐</span>` : ''}`;
  ov.innerHTML = schoolHeader('📋 성적표') + `
    <div style="padding:28px 16px 120px;text-align:center;">
      <div style="background:#fff;border:2px solid #fde68a;border-radius:24px;padding:26px 18px;box-shadow:0 6px 24px #F59E0B22;">
        <div style="font-size:13px;color:#777;font-weight:900;margin-bottom:4px;">평균 ${avg}점</div>
        <div style="font-size:64px;font-weight:900;color:#F59E0B;line-height:1;">${rank}</div>
        <div style="font-size:18px;font-weight:900;color:#111;margin:12px 0;line-height:1.55;">${rewardText}</div>
        <div style="font-size:12px;color:#777;line-height:1.5;margin-bottom:20px;">국어 ${vals[0]} · 암기카드 ${vals[1]} · 체육 ${vals[2]}<br>✨ 우등생조각 100개 = ⭐ 우등생별 1개</div>
        <button onclick="renderSchoolHome()" style="width:100%;padding:14px;border:none;border-radius:14px;background:linear-gradient(135deg,#F59E0B,#FF6B9D);color:#fff;font-size:15px;font-weight:900;font-family:'Noto Sans KR',sans-serif;cursor:pointer;">확인</button>
      </div>
    </div>`;
  showBagToast(`${starReward ? '⭐ 우등생별 ' + starReward + '개 · ' : ''}✨ 우등생조각 ${fragmentReward}개 획득!`);
}

// 기존 맵 함수에 학교 버튼만 안전하게 추가
(function patchSchoolButtonIntoMap(){
  const oldOpenPlace = window.openPlace || openPlace;
  window.openPlace = function(id) {
    ensureSchoolButton();
    oldOpenPlace(id);
    const btn = document.getElementById('btn-school-games');
    if (btn) btn.style.display = (id === 'school') ? 'block' : 'none';
  };
  try { ensureSchoolButton(); } catch(e) {}
})();
...
/* ===== POCA IMAGE LINK FIX v12 ===== */
(function(){

  function getPopupImg(){
    try{
      return (
        document.querySelector('#result-popup img') ||
        document.querySelector('#craft-popup img') ||
        document.querySelector('#cloth-popup img') ||
  ...
  function fixHook(fnName, label){
    ...
    window[fnName] = function(){
      ...
      setTimeout(()=>{
        ...
        btn.onclick = ()=>{
          const text = prompt('자랑할 내용을 입력하세요 😆') || '';
          if(!text) return;

          const img = getPopupImg();

          if(window.__addTimeline){
            window.__addTimeline(label + ' - ' + text, img);
          }

          const openBtn = document.getElementById('timeline-btn');
  ...
  // override existing hooks safely
  fixHook('showGachaResult','🎴 가챠 결과');
  fixHook('showResult','📚 성적 결과');
  fixHook('showCraftResult','🧪 제작 결과');

})();



/* ===== POCA CLOTH EQUIP FIX v1 ===== */
(function(){
  ...
  function findClothByIdOrName(clothId, itemName){
    if (typeof CLOTH_ITEMS === 'undefined') return null;
    const targetName = cleanName(itemName);
    return CLOTH_ITEMS.find(c =>
      c.id === clothId ||
      c.name === itemName ||
      cleanName(c.name) === targetName
    ) || null;
  }

  function findBagCloth(cloth, clothId, itemName){
    if (typeof bagItems === 'undefined') return null;
    const targetName = cleanName(itemName);
    return bagItems.find(i => {
      if (!i || i.type !== 'cloth') return false;
      return (
        i.clothId === clothId ||
        i.clothId === cloth?.id ||
        i.name === itemName ||
        cleanName(i.name) === targetName ||
        cleanName(i.name) === cleanName(cloth?.name)
      );
    }) || null;
  }
  ...
  window.equipCloth = function(clothId, itemName){
    try {
      const cloth = findClothByIdOrName(clothId, itemName);
      if (!cloth) {
        if (typeof showBagToast === 'function') showBagToast('의상 정보를 찾지 못했어요!');
        else alert('의상 정보를 찾지 못했어요!');
        return;
      }

      const item = findBagCloth(cloth, clothId, itemName);
      if (!item) {
        if (typeof showBagToast === 'function') showBagToast('보유한 의상이 아니에요!');
        else alert('보유한 의상이 아니에요!');
        return;
      }
      ...
      equippedCloth = {
        id: cloth.id,
        clothId: cloth.id,
        name: item.name || cloth.name,
        stat: cloth.stat || item.stat || '',
        statVal: item.statVal || cloth.statVal || 0,
        img: item.img || cloth.img || '',
      ...
      if (typeof renderBag === 'function') renderBag();
      if (typeof renderHomeLevelPanel === 'function') renderHomeLevelPanel();
      if (typeof renderInventoryDisplay === 'function') renderInventoryDisplay();
      if (typeof schedulePocaServerSave === 'function') schedulePocaServerSave('equipCloth');
      ...
      if (typeof showBagToast === 'function') showBagToast(`👗 ${item.name || cloth.name} 착용 완료!`);
      else alert(`${item.name || cloth.name} 착용 완료!`);

    } catch(e) {
      console.error('equipCloth fix error:', e);
      if (typeof showBagToast === 'function') showBagToast('옷 착용 중 오류가 났어요!');
      else alert('옷 착용 중 오류가 났어요!');
    }
  };

  window.unequipCloth = function(){
    equippedCloth = null;
    localStorage.setItem('ph_equippedCloth', JSON.stringify(null));
    if (typeof saveAll === 'function') saveAll();
    if (typeof renderBag === 'function') renderBag();
    if (typeof showBagToast === 'function') showBagToast('옷을 벗었어요!');
  };

  // 기존 팝업의 착용 버튼 onclick이 꼬였을 때도 클릭을 살리는 안전 장치
  document.addEventListener('click', function(e){
    const btn = e.target.closest('button');
    if (!btn) return;
    const text = btn.innerText || btn.textContent || '';
    if (!text.includes('착용')) return;

    const overlay = btn.closest('#cloth-detail-overlay, #sewing-recipe-overlay, #bag-detail-overlay');
    if (!overlay) return;

    // 이미 원래 onclick이 정상 실행될 수 있으므로 아주 짧게 기다렸다가 변화가 없으면 보정
    setTimeout(() => {
      const before = localStorage.getItem('ph_equippedCloth');
      if (before && before !== 'null') return;

      const itemNameEl = overlay.querySelector('[data-cloth-name]');
      const itemName = itemNameEl?.getAttribute('data-cloth-name') || 
                       overlay.innerText.match(/야구점퍼|후드|원피스|드레스|교복|재킷|자켓|점퍼/)?.[0] || '';

      if (itemName && typeof CLOTH_ITEMS !== 'undefined') {
        const cloth = CLOTH_ITEMS.find(c => cleanName(c.name).includes(cleanName(itemName)) || cleanName(itemName).includes(cleanName(c.name)));
        if (cloth) window.equipCloth(cloth.id, itemName);
      }
    }, 80);
  }, true);
  ...
  // 저장된 착용 상태 복구
  try {
    const saved = localStorage.getItem('ph_equippedCloth');
    if (saved && saved !== 'null') equippedCloth = JSON.parse(saved);
  } catch(e) {}
...
/* ===========================
   POCA EMERGENCY FIX v16
   SERVER BUTTON + CLOTH SYNC RESET
=========================== */
(function(){

/* 1. HARD REMOVE SERVER UI */
const style = document.createElement('style');
style.textContent = `
#firebase-test-panel,
.firebase-test-panel,
div[onclick*="firebase"],
...
/* 2. KILL RECREATION */
window.addFirebaseTestPanel = function(){ return; };

const _setTimeout = window.setTimeout;
window.setTimeout = function(fn, t){
  const s = String(fn);
  if (s.includes('firebase') || s.includes('Server') || s.includes('server')) return 0;
  return _setTimeout(fn, t);
};

/* 3. CLOTH TOGGLE SAFE FIX */
window.toggleClothFix = function(cloth){
  if (!window.equippedCloth) window.equippedCloth = null;

  const isEquipped = window.equippedCloth && window.equippedCloth.id === cloth.id;

  if (isEquipped) {
    window.equippedCloth = null;
    if (typeof showBagToast === 'function') showBagToast('옷 해제 완료!');
  } else {
    window.equippedCloth = cloth;
    if (typeof showBagToast === 'function') showBagToast('옷 착용 완료!');
  }

  try { localStorage.setItem('ph_equippedCloth', JSON.stringify(window.equippedCloth)); } catch(e){}
  if (typeof renderBag === 'function') renderBag();
};
